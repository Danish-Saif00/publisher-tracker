...
-- Company Admin own-company Factory Reset:
-- Preserve the Company shell, current Company Admin identity/membership,
-- company subscription/plan assignment, and invoice history.
-- Continue purging company-scoped operational data and child identities.
-- Platform/global Factory Reset semantics are intentionally unchanged.
...
declare
  ...
  preserved_subscription_count integer := 0;
  preserved_invoice_count integer := 0;
begin
  ...
  if (
    ...
    from public.company_subscriptions as subscription
    where subscription.company_id = target_company_id
  ) is distinct from preserved_subscription_count then
    raise exception
      using
        errcode = '23514',
        message =
          'Company factory reset postcondition failed: subscription count changed.';
  end if;
  ...
  if (
    ...
    from public.billing_invoices as invoice
    where invoice.company_id = target_company_id
  ) is distinct from preserved_invoice_count then
    raise exception
      using
        errcode = '23514',
        message =
          'Company factory reset postcondition failed: invoice count changed.';
  end if;
