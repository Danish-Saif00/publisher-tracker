...
import { createSupabaseAccessTokenVerifier, createSupabaseUserInvitationGateway, } from '@affiliate-tracker/auth';
import { createDatabase } from '@affiliate-tracker/database';
import { createLogger, serializeError, } from '@affiliate-tracker/observability';
...
async function closeDatabaseAfterStartupFailure(database, logger) {
    try {
...
function combineShutdownErrors(currentError, nextError) {
    if (currentError === undefined) {
        return nextError;
    }
    return new AggregateError([currentError, nextError], 'Multiple API resources failed to close cleanly.');
}
async function bootstrap() {
    ...
    try {
        ...
        const invitationEmailOutboxRepository = createInvitationEmailOutboxRepository(database);
        const invitationEmailPayloadCipher = createEmailPayloadCipher(config.security.dataEncryptionKey);
        const companyInvitationsService = createCompanyInvitationsService(companyInvitationsRepository, invitationGateway, invitationEmailOutboxRepository, invitationEmailPayloadCipher, config.frontend.publicUrl);
        const tenantAdministrationRepository = createTenantAdministrationRepository(database);
        const tenantAdministrationService = createTenantAdministrationService(tenantAdministrationRepository);
        ...
        const companyOperationsService = createCompanyOperationsService(companyOperationsRepository, credentialCipher, companyMailTransport);
        const conversionPostbacksRepository = createConversionPostbacksRepository(database);
        const conversionPostbacksService = createConversionPostbacksService(conversionPostbacksRepository);
