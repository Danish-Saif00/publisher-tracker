...
const ALLOWED_OFFER_TRANSITIONS = {
...
function normalizeOptionalNullableText(value, fieldName, maximumLength) {
    ...
    if (normalizedValue.length === 0 || normalizedValue.length > maximumLength) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, `${fieldName} must contain 1 to ${String(maximumLength)} characters or be null.`);
    }
    return normalizedValue;
}
function normalizeOfferCode(value) {
    ...
    if (normalizedValue.length < 2 ||
        ...
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'Offer code must contain 2 to 80 lowercase letters, numbers, underscores, or hyphens.');
    }
    return normalizedValue;
}
function normalizeUrl(value, fieldName) {
    ...
    if (normalizedValue.length < 8 || normalizedValue.length > 2048) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, `${fieldName} must contain 8 to 2048 characters.`);
    }
    let url;
    try {
...
function normalizeAmount(value, fieldName) {
    if (!Number.isSafeInteger(value) || value < 1 || value > MAX_AMOUNT_MINOR) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, `${fieldName} must be an integer between 1 and ${String(MAX_AMOUNT_MINOR)}.`);
    }
    return value;
}
function normalizeCurrency(value, fieldName) {
    ...
    if (!CURRENCY_PATTERN.test(normalizedValue)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, `${fieldName} must be a three-letter ISO currency code.`);
    }
    return normalizedValue;
}
...
function normalizePayoutProfileInput(input) {
    const mode = normalizePayoutMode(input.mode);
    if (mode === 'fixed_member') {
        if (input.fixedPayoutAmountMinor === undefined ||
            input.fixedPayoutAmountMinor === null ||
            input.payoutCurrency === undefined ||
            input.payoutCurrency === null) {
            throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'fixed_member mode requires fixedPayoutAmountMinor and payoutCurrency.');
        }
        return Object.freeze({
            mode,
            fixedPayoutAmountMinor: normalizeAmount(input.fixedPayoutAmountMinor, 'fixedPayoutAmountMinor'),
            payoutCurrency: normalizeCurrency(input.payoutCurrency, 'payoutCurrency'),
        });
    }
    if ((input.fixedPayoutAmountMinor !== undefined && input.fixedPayoutAmountMinor !== null) ||
        (input.payoutCurrency !== undefined && input.payoutCurrency !== null)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'per_offer mode cannot define a fixed payout amount or currency.');
    }
    return Object.freeze({
        mode,
...
function normalizeManualPayoutPair(amount, currency) {
    ...
    if (amount === undefined || currency === undefined) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'manualPayoutAmountMinor and manualPayoutCurrency must be provided together.');
    }
    ...
    if (amount === null || currency === null) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'Manual payout amount and currency must both be values or both be null.');
    }
    return Object.freeze({
...
async function requireActiveCompany(repository, context, companyId) {
    ...
    if (company.status !== 'active') {
        throw new ApiHttpError('OFFER_COMPANY_INACTIVE', 409, 'Offers and payout settings require an active company.');
    }
}
async function requireActiveNetworkAccount(repository, context, companyId, networkAccountId) {
    const account = await repository.getNetworkAccount(context, companyId, networkAccountId);
    if (account?.status !== 'active') {
        throw new ApiHttpError('OFFER_NETWORK_ACCOUNT_INVALID', 409, 'The offer requires an active network account from the same company.');
    }
}
function isScopedOfferReader(identity) {
    return (identity.subject.platformRole === undefined &&
        (identity.companyMembership?.role === 'manager' ||
            identity.companyMembership?.role === 'publisher'));
}
function getManagerMembershipId(identity) {
    return identity.companyMembership?.role === 'manager'
        ? identity.companyMembership.membershipId
        : undefined;
}
function offersAreEquivalent(current, next) {
    return (current.networkAccountId === next.networkAccountId &&
...
function assertOfferTransition(current, next) {
    if (current === next) {
        return;
    }
    if (!ALLOWED_OFFER_TRANSITIONS[current].includes(next)) {
        throw new ApiHttpError('OFFER_STATUS_TRANSITION_INVALID', 409, `An offer cannot transition from ${current} to ${next}.`);
    }
}
function assignmentsAreEquivalent(current, next) {
...
function assertAssignmentTransition(current, next) {
    if (current === next) {
        return;
    }
    if (!ALLOWED_ASSIGNMENT_TRANSITIONS[current].includes(next)) {
        throw new ApiHttpError('OFFER_ASSIGNMENT_STATUS_TRANSITION_INVALID', 409, `An offer assignment cannot transition from ${current} to ${next}.`);
    }
}
async function requireOffer(repository, context, companyId, offerId, visibleToUserId) {
...
export function createOffersPayoutService(repository) {
    return Object.freeze({
        async createOffer(identity, requestId, companyIdValue, input) {
            ...
            const offer = await repository.createOffer(context, companyId, {
                ...
                externalOfferId: normalizeOptionalNullableText(input.externalOfferId, 'externalOfferId', MAX_EXTERNAL_OFFER_ID_LENGTH) ?? null,
            ...
            if (offer === undefined) {
                throw new ApiHttpError('OFFER_CONFLICT', 409, 'An offer with this code or external network identifier already exists.');
            }
            return offer;
        },
        ...
        async getOffer(identity, requestId, companyIdValue, offerIdValue) {
            ...
            const context = createRepositoryContext(identity, requestId, companyId);
            await requireActiveCompany(repository, context, companyId);
            return requireOffer(repository, context, companyId, offerId, isScopedOfferReader(identity) ? identity.actor.userId : undefined);
        },
        async updateOffer(identity, requestId, companyIdValue, offerIdValue, input) {
            ...
            if (input.externalOfferId === undefined &&
                input.name === undefined &&
                input.description === undefined &&
                input.destinationUrl === undefined &&
                input.status === undefined) {
                throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'At least one offer field must be provided.');
            }
            const status = input.status === undefined ? current.status : normalizeOfferStatus(input.status);
            ...
            const next = Object.freeze({
                ...
                code: current.code,
                externalOfferId: input.externalOfferId === undefined
                    ? current.externalOfferId
                    : (normalizeOptionalNullableText(input.externalOfferId, 'externalOfferId', MAX_EXTERNAL_OFFER_ID_LENGTH) ?? null),
            ...
            if (offersAreEquivalent(current, next)) {
                throw new ApiHttpError('OFFER_UNCHANGED', 409, 'The offer already contains the requested values.');
            }
            const updated = await repository.updateOffer(context, current, next);
            if (updated === undefined) {
                throw new ApiHttpError('OFFER_UPDATE_CONFLICT', 409, 'The offer changed or conflicted before this request completed.');
            }
            return updated;
        },
        async upsertPayoutProfile(identity, requestId, companyIdValue, membershipIdValue, input) {
            ...
            if (membership?.status !== 'active') {
                throw new ApiHttpError('PAYOUT_PROFILE_MEMBER_INVALID', 409, 'A payout profile requires an active Manager or Publisher membership.');
            }
            ...
            if (current === undefined) {
                const created = await repository.createPayoutProfile(context, companyId, membershipId, normalizedInput);
                if (created === undefined) {
                    throw new ApiHttpError('PAYOUT_PROFILE_UPDATE_CONFLICT', 409, 'The payout profile was created by another request.');
                }
                return created;
            }
            if (current.mode === normalizedInput.mode &&
                ...
                throw new ApiHttpError('PAYOUT_PROFILE_UNCHANGED', 409, 'The payout profile already contains the requested values.');
            }
            if (normalizedInput.mode === 'per_offer') {
                const incompleteAssignments = await repository.countOpenAssignmentsMissingManualPayout(context, companyId, membershipId);
                if (incompleteAssignments > 0) {
                    throw new ApiHttpError('PAYOUT_PROFILE_ASSIGNMENTS_INCOMPLETE', 409, 'Every open assignment must define a manual payout before switching to per_offer mode.');
                }
            }
            const updated = await repository.updatePayoutProfile(context, current, normalizedInput);
            if (updated === undefined) {
                throw new ApiHttpError('PAYOUT_PROFILE_UPDATE_CONFLICT', 409, 'The payout profile changed before this request completed.');
            }
            return updated;
        },
        ...
        async getOwnPayoutProfile(identity, requestId, companyIdValue) {
            ...
            if (membershipId === undefined) {
                throw new ApiHttpError('PAYOUT_PROFILE_NOT_FOUND', 404, 'The current user does not have a payout profile.');
            }
            const context = createRepositoryContext(identity, requestId, companyId);
            await requireActiveCompany(repository, context, companyId);
        ...
        async createAssignment(identity, requestId, companyIdValue, offerIdValue, input) {
            ...
            await requireActiveCompany(repository, context, companyId);
            const offer = await requireOffer(repository, context, companyId, offerId, managerMembershipId === undefined ? undefined : identity.actor.userId);
            if (offer.status === 'archived') {
                throw new ApiHttpError('OFFER_ARCHIVED', 409, 'An archived offer cannot receive assignments.');
            }
            const membership = await repository.getEligibleMembership(context, companyId, membershipId);
            if (membership?.status !== 'active') {
                throw new ApiHttpError('OFFER_ASSIGNMENT_MEMBER_INVALID', 409, 'An offer can only be assigned to an active membership.');
            }
            if (managerMembershipId === undefined) {
                if (membership.role !== 'manager') {
                    throw new ApiHttpError('OFFER_ASSIGNMENT_MEMBER_INVALID', 409, 'A Company Admin can assign Offers to active Manager memberships only.');
                }
            }
            else {
                if (membership.role !== 'publisher' ||
                    (membership.invitedBy !== identity.actor.userId &&
                        !(await repository.listOfferAssignments(context, companyId, offerId, managerMembershipId)).some((assignment) => assignment.membershipId === membershipId))) {
                    throw new ApiHttpError('OFFER_ASSIGNMENT_MEMBER_INVALID', 403, 'A Manager can assign an Offer only to a Publisher within their own scope.');
                }
            }
            const profile = await repository.getPayoutProfile(context, companyId, membershipId);
            if (profile === undefined) {
                throw new ApiHttpError('PAYOUT_PROFILE_NOT_FOUND', 409, 'Create the member payout profile before assigning an offer.');
            }
            const manualPayout = normalizeManualPayoutPair(input.manualPayoutAmountMinor, input.manualPayoutCurrency);
            if (profile.mode === 'per_offer' &&
                (manualPayout.amountMinor === null || manualPayout.currency === null)) {
                throw new ApiHttpError('OFFER_ASSIGNMENT_PAYOUT_REQUIRED', 409, 'per_offer payout mode requires a manual payout on every open assignment.');
            }
            const assignment = await repository.createAssignment(context, companyId, offerId, membershipId, {
            ...
            if (assignment === undefined) {
                throw new ApiHttpError('OFFER_ASSIGNMENT_CONFLICT', 409, 'This member is already assigned to the offer.');
            }
            return assignment;
        },
        async listOfferAssignments(identity, requestId, companyIdValue, offerIdValue) {
            ...
            const managerMembershipId = getManagerMembershipId(identity);
            await requireActiveCompany(repository, context, companyId);
            await requireOffer(repository, context, companyId, offerId, managerMembershipId === undefined ? undefined : identity.actor.userId);
            return repository.listOfferAssignments(context, companyId, offerId, managerMembershipId);
        },
        async updateAssignment(identity, requestId, companyIdValue, offerIdValue, assignmentIdValue, input) {
            const companyId = normalizeUuid(companyIdValue, 'Company ID');
            const offerId = normalizeUuid(offerIdValue, 'Offer ID');
            ...
            await requireActiveCompany(repository, context, companyId);
            await requireOffer(repository, context, companyId, offerId, managerMembershipId === undefined ? undefined : identity.actor.userId);
            const current = await repository.getAssignment(context, companyId, offerId, assignmentId, managerMembershipId);
            if (current === undefined) {
                throw new ApiHttpError('OFFER_ASSIGNMENT_NOT_FOUND', 404, 'The requested offer assignment was not found.');
            }
            if (input.status === undefined &&
                input.manualPayoutAmountMinor === undefined &&
                input.manualPayoutCurrency === undefined) {
                throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'At least one offer-assignment field must be provided.');
            }
            const status = input.status === undefined ? current.status : normalizeAssignmentStatus(input.status);
            assertAssignmentTransition(current.status, status);
            const manualPayout = input.manualPayoutAmountMinor === undefined && input.manualPayoutCurrency === undefined
                ? {
            ...
            if (profile === undefined) {
                throw new ApiHttpError('PAYOUT_PROFILE_NOT_FOUND', 409, 'The assigned member payout profile is unavailable.');
            }
            if (status !== 'revoked' &&
            ...
            if (assignmentsAreEquivalent(current, next)) {
                throw new ApiHttpError('OFFER_ASSIGNMENT_UNCHANGED', 409, 'The offer assignment already contains the requested values.');
            }
            const updated = await repository.updateAssignment(context, current, next);
            if (updated === undefined) {
                throw new ApiHttpError('OFFER_ASSIGNMENT_UPDATE_CONFLICT', 409, 'The offer assignment changed before this request completed.');
            }
            return updated;
        },
    });
}
//# sourceMappingURL=offers-payout.service.js.map