...
function normalizeToken(value) {
    ...
    if (!TOKEN_PATTERN.test(normalizedValue)) {
        throw new ApiHttpError('INVITATION_NOT_FOUND', 404, 'The invitation is invalid or unavailable.');
    }
    return normalizedValue;
}
...
function assertCompanyContext(identity, companyId) {
...
function requireActorEmail(identity) {
    const email = identity.actor.email;
    if (email === undefined) {
        throw new ApiHttpError('INVITATION_RECIPIENT_MISMATCH', 403, 'The authenticated account does not have an email address.');
    }
...
function assertInvitationRolePermission(identity, companyId, targetRole) {
    assertCompanyRole(identity.subject, identity.companyMembership, companyId, ['company_admin', 'manager']);
    if (isPlatformSuperAdmin(identity.subject)) {
        if (targetRole === 'company_admin') {
            return;
        }
        throw new ApiHttpError('PLATFORM_SUPER_ADMIN_ROLE_ASSIGNMENT_FORBIDDEN', 403, 'A Platform Super Admin can only create or manage Company Admin invitations.');
    }
    ...
    throw new ApiHttpError('INVITATION_ROLE_ASSIGNMENT_FORBIDDEN', 403, 'The current role cannot create or manage an invitation for the requested role.');
}
export function createCompanyInvitationsService(repository, linkGateway, outboxRepository, payloadCipher, publicAppUrl) {
    const normalizedAppUrl = publicAppUrl.trim().replace(/\/+$/u, '');
    async function requireActiveCompany(context, companyId) {
    ...
    async function queueDelivery(context, companyName, invitation, token) {
        ...
        try {
            ...
            throw new ApiHttpError('INVITATION_LINK_GENERATION_FAILED', 502, 'The invitation was saved, but its secure access link could not be generated.', {
                cause: error,
            });
        }
        ...
        const idempotencyKey = `company-invitation:${invitation.id}:${hashToken(token)}`;
        ...
        const queuedInvitation = await repository.getInvitationById(context, invitation.companyId, invitation.id);
        if (queuedInvitation === undefined) {
            throw new Error('The invitation changed after its email notification was queued.');
        }
        return queuedInvitation;
    }
    async function requireInvitationForRecipient(identity, requestId, tokenValue) {
        ...
        if (invitation.status === 'accepted') {
            throw new ApiHttpError('INVITATION_ALREADY_ACCEPTED', 409, 'This invitation has already been accepted.');
        }
        if (invitation.status === 'revoked') {
            throw new ApiHttpError('INVITATION_REVOKED', 410, 'This invitation has been revoked.');
        }
        if (isExpired(invitation)) {
            throw new ApiHttpError('INVITATION_EXPIRED', 410, 'This invitation has expired. Ask an administrator to resend it.');
        }
        ...
        if (actorEmail !== invitation.email ||
            ...
            throw new ApiHttpError('INVITATION_RECIPIENT_MISMATCH', 403, 'This invitation belongs to a different account.');
        }
    ...
    return Object.freeze({
        async createInvitation(identity, requestId, companyIdValue, input) {
            ...
            if (existingUser !== undefined) {
                const membership = await repository.getMembershipByUserId(context, companyId, existingUser.userId);
                if (membership !== undefined && membership.status !== 'invited') {
                    throw new ApiHttpError('MEMBERSHIP_CONFLICT', 409, 'This user already has a non-invited membership in the company.');
                }
            }
            ...
            if (invitation === undefined) {
                throw new ApiHttpError('INVITATION_CONFLICT', 409, 'A pending or accepted invitation already exists for this email address.');
            }
            return queueDelivery(context, company.name, invitation, token);
        },
        ...
        async resendInvitation(identity, requestId, companyIdValue, invitationIdValue) {
            ...
            if (current === undefined) {
                throw new ApiHttpError('INVITATION_NOT_FOUND', 404, 'The requested invitation was not found.');
            }
            if (current.status !== 'pending') {
                throw new ApiHttpError('INVITATION_NOT_PENDING', 409, 'Only a pending invitation can be resent.');
            }
            ...
            if (rotated === undefined) {
                throw new ApiHttpError('INVITATION_UPDATE_CONFLICT', 409, 'The invitation changed before it could be resent.');
            }
            return queueDelivery(context, company.name, rotated, token);
        },
        async revokeInvitation(identity, requestId, companyIdValue, invitationIdValue) {
            ...
            if (invitation === undefined) {
                throw new ApiHttpError('INVITATION_NOT_PENDING', 409, 'Only a pending invitation can be revoked.');
            }
            await outboxRepository.cancelPending(context, invitation.id);
            return invitation;
        },
        ...
        async acceptInvitation(identity, requestId, tokenValue) {
            ...
            const accepted = await repository.acceptInvitation(createContext(identity, requestId), result.tokenHash, requireActorEmail(identity));
            if (accepted === undefined) {
                throw new ApiHttpError('INVITATION_ACCEPTANCE_CONFLICT', 409, 'The invitation changed before it could be accepted.');
            }
            return accepted;
        },
    });
}
//# sourceMappingURL=company-invitations.service.js.map