...
function normalizeHostname(value) {
    ...
    if (normalizedValue.length < 4 ||
        ...
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'hostname must be a valid lowercase fully qualified domain name.');
    }
    return normalizedValue;
}
function normalizeProviderCode(value) {
    ...
    if (normalizedValue.length < 2 ||
        ...
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'Provider code must contain 2 to 80 lowercase letters, numbers, or single underscores.');
    }
    return normalizedValue;
}
...
function normalizeOptionalUrl(value, fieldName) {
    ...
    if (url.protocol !== 'http:' && url.protocol !== 'https:') {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, `${fieldName} must use the http or https protocol.`);
    }
    return url.toString();
}
function normalizeTrackingDomainStatus(value) {
    switch (value) {
...
function normalizeTenantTrackingDomainStatus(value) {
    switch (value) {
        case 'active':
        case 'suspended':
        case 'archived':
            return value;
        default:
            throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'Tenant tracking-domain status must be active, suspended, or archived.');
    }
}
function normalizeNetworkProviderStatus(value, code) {
    switch (value) {
        case 'active':
        case 'archived':
            return value;
...
function normalizeNetworkAccountStatus(value, code) {
    switch (value) {
        case 'active':
        case 'suspended':
        case 'archived':
...
async function requireActiveCompany(repository, context, companyId) {
    ...
    if (company.status !== 'active') {
        throw new ApiHttpError('TRACKING_NETWORK_COMPANY_INACTIVE', 409, 'Tracking and network configuration requires an active company.');
    }
    return company;
}
async function requireTrackingDomain(repository, context, domainId, companyId) {
    const domain = await repository.getTrackingDomain(context, domainId, companyId);
    if (domain === undefined) {
        throw new ApiHttpError('TRACKING_DOMAIN_NOT_FOUND', 404, 'The requested tracking domain was not found.');
    }
    return domain;
}
async function requireNetworkProvider(repository, context, providerId) {
    const provider = await repository.getNetworkProvider(context, providerId);
    if (provider === undefined) {
        throw new ApiHttpError('NETWORK_PROVIDER_NOT_FOUND', 404, 'The requested network provider was not found.');
    }
    return provider;
}
async function requireActiveNetworkProvider(repository, context, providerId) {
    const provider = await requireNetworkProvider(repository, context, providerId);
    if (provider.status !== 'active') {
        throw new ApiHttpError('NETWORK_PROVIDER_ARCHIVED', 409, 'An archived network provider cannot receive new accounts.');
    }
    return provider;
}
async function requireNetworkAccount(repository, context, accountId, companyId) {
    const account = await repository.getNetworkAccount(context, accountId, companyId);
    if (account === undefined) {
        throw new ApiHttpError('NETWORK_ACCOUNT_NOT_FOUND', 404, 'The requested network account was not found.');
    }
    return account;
}
function trackingDomainsAreEquivalent(current, next) {
...
function providersAreEquivalent(current, next) {
    return (current.name === next.name &&
        current.status === next.status &&
        current.websiteUrl === next.websiteUrl &&
        current.documentationUrl === next.documentationUrl);
}
function accountsAreEquivalent(current, next) {
    return (current.name === next.name &&
        current.externalAccountId === next.externalAccountId &&
        current.status === next.status);
}
function assertNetworkAccountTransition(currentStatus, nextStatus) {
    if (currentStatus === 'archived') {
        throw new ApiHttpError('NETWORK_ACCOUNT_ARCHIVED', 409, 'An archived network account is immutable.');
    }
    const valid = currentStatus === nextStatus ||
    ...
    if (!valid) {
        throw new ApiHttpError('NETWORK_ACCOUNT_STATUS_TRANSITION_INVALID', 409, `A network account cannot transition from ${currentStatus} to ${nextStatus}.`);
    }
}
export function createTrackingNetworksService(repository, options = {}) {
    const getNow = options.now ?? (() => new Date());
    const createVerificationToken = options.createVerificationToken ?? (() => randomBytes(32).toString('base64url'));
    return Object.freeze({
        async createTrackingDomain(identity, requestId, companyIdValue, input) {
            ...
            if (domain === undefined) {
                throw new ApiHttpError('TRACKING_DOMAIN_HOSTNAME_CONFLICT', 409, 'This tracking hostname or verification token is already registered.');
            }
            return domain;
        },
        ...
        async updateCompanyTrackingDomain(identity, requestId, companyIdValue, domainIdValue, input) {
            ...
            if (current.status === 'archived') {
                throw new ApiHttpError('TRACKING_DOMAIN_ARCHIVED', 409, 'An archived tracking domain is immutable.');
            }
            if (input.hostname === undefined &&
                input.status === undefined &&
                input.isPrimary === undefined) {
                throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'At least one tracking-domain field must be provided.');
            }
            ...
            if (input.hostname !== undefined) {
                ...
                if (hostname !== current.hostname) {
                    if (current.status !== 'pending_verification') {
                        throw new ApiHttpError('TRACKING_DOMAIN_HOSTNAME_LOCKED', 409, 'A verified or suspended tracking-domain hostname cannot be changed.');
                    }
            ...
            if (isPrimary && (status !== 'active' || verifiedAt === null)) {
                throw new ApiHttpError('TRACKING_DOMAIN_UNVERIFIED', 409, 'Only an active verified tracking domain can be primary.');
            }
            ...
            if (trackingDomainsAreEquivalent(current, next)) {
                throw new ApiHttpError('TRACKING_DOMAIN_UNCHANGED', 409, 'The tracking domain already contains the requested values.');
            }
            const updated = await repository.updateTrackingDomain(context, current, next, 'tracking_domain.updated');
            if (updated === undefined) {
                throw new ApiHttpError('TRACKING_DOMAIN_UPDATE_CONFLICT', 409, 'The tracking domain changed or its hostname conflicted before this request completed.');
            }
            return updated;
        },
        ...
        async updatePlatformTrackingDomainStatus(identity, requestId, domainIdValue, input) {
            ...
            if (current.status === input.status) {
                throw new ApiHttpError('TRACKING_DOMAIN_UNCHANGED', 409, 'The tracking domain already has the requested status.');
            }
            ...
            const updated = await repository.updateTrackingDomain(createRepositoryContext(identity, requestId, current.companyId), current, next, input.status === 'active' ? 'tracking_domain.verified' : 'tracking_domain.status_updated');
            if (updated === undefined) {
                throw new ApiHttpError('TRACKING_DOMAIN_UPDATE_CONFLICT', 409, 'The tracking domain changed before this request completed.');
            }
            return updated;
        },
        async createNetworkProvider(identity, requestId, input) {
            assertPlatformSuperAdmin(identity.subject);
            const provider = await repository.createNetworkProvider(createRepositoryContext(identity, requestId), Object.freeze({
                ...
                documentationUrl: normalizeOptionalUrl(input.documentationUrl, 'documentationUrl') ?? null,
            }));
            if (provider === undefined) {
                throw new ApiHttpError('NETWORK_PROVIDER_CODE_CONFLICT', 409, 'A network provider with this code already exists.');
            }
            return provider;
        },
        async listPlatformNetworkProviders(identity, requestId, status) {
            assertPlatformSuperAdmin(identity.subject);
            return repository.listNetworkProviders(createRepositoryContext(identity, requestId), status === undefined
                ? undefined
        ...
        async getPlatformNetworkProvider(identity, requestId, providerIdValue) {
            assertPlatformSuperAdmin(identity.subject);
            return requireNetworkProvider(repository, createRepositoryContext(identity, requestId), normalizeUuid(providerIdValue, 'Network provider ID'));
        },
        async updateNetworkProvider(identity, requestId, providerIdValue, input) {
            ...
            if (current.status === 'archived') {
                throw new ApiHttpError('NETWORK_PROVIDER_ARCHIVED', 409, 'An archived network provider is immutable.');
            }
            if (input.name === undefined &&
                input.status === undefined &&
                input.websiteUrl === undefined &&
                input.documentationUrl === undefined) {
                throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'At least one network-provider field must be provided.');
            }
            const next = Object.freeze({
                code: current.code,
            ...
            if (providersAreEquivalent(current, next)) {
                throw new ApiHttpError('NETWORK_PROVIDER_UNCHANGED', 409, 'The network provider already contains the requested values.');
            }
            if (next.status === 'archived') {
                const openAccounts = await repository.countOpenNetworkAccountsForProvider(context, providerId);
                if (openAccounts > 0) {
                    throw new ApiHttpError('NETWORK_PROVIDER_IN_USE', 409, 'A network provider with open company accounts cannot be archived.');
                }
            }
            const updated = await repository.updateNetworkProvider(context, current, next);
            if (updated === undefined) {
                throw new ApiHttpError('NETWORK_PROVIDER_UPDATE_CONFLICT', 409, 'The network provider changed before this request completed.');
            }
            return updated;
        },
        ...
        async createNetworkAccount(identity, requestId, companyIdValue, input) {
            ...
            const account = await repository.createNetworkAccount(context, companyId, {
                providerId,
                name: normalizeRequiredText(input.name, 'name', 2, 160),
                externalAccountId: normalizeOptionalNullableText(input.externalAccountId, 'externalAccountId', 255) ?? null,
                status: 'active',
            });
            if (account === undefined) {
                throw new ApiHttpError('NETWORK_ACCOUNT_CONFLICT', 409, 'A network account with this provider name or external account ID already exists.');
            }
            return account;
        },
        ...
        async updateCompanyNetworkAccount(identity, requestId, companyIdValue, accountIdValue, input) {
            ...
            if (input.name === undefined &&
                ...
                input.status === undefined) {
                throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'At least one network-account field must be provided.');
            }
            ...
            if (accountsAreEquivalent(current, next)) {
                throw new ApiHttpError('NETWORK_ACCOUNT_UNCHANGED', 409, 'The network account already contains the requested values.');
            }
            const updated = await repository.updateNetworkAccount(context, current, next, 'network_account.updated');
            if (updated === undefined) {
                throw new ApiHttpError('NETWORK_ACCOUNT_UPDATE_CONFLICT', 409, 'The network account changed or conflicted before this request completed.');
            }
            return updated;
        },
        ...
        async updatePlatformNetworkAccountStatus(identity, requestId, accountIdValue, input) {
            ...
            if (current.status === nextStatus) {
                throw new ApiHttpError('NETWORK_ACCOUNT_UNCHANGED', 409, 'The network account already has the requested status.');
            }
            const updated = await repository.updateNetworkAccount(createRepositoryContext(identity, requestId, current.companyId), current, {
                providerId: current.providerId,
                name: current.name,
            ...
            if (updated === undefined) {
                throw new ApiHttpError('NETWORK_ACCOUNT_UPDATE_CONFLICT', 409, 'The network account changed before this request completed.');
            }
            return updated;
        },
    });
}
//# sourceMappingURL=tracking-networks.service.js.map