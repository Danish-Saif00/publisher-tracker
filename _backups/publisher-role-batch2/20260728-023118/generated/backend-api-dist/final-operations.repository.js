...
function mapConversionStatus(value) {
    switch (value) {
        case 'pending':
        case 'approved':
        case 'rejected':
...
function mapProxyOutcome(value) {
    switch (value) {
        case 'not_checked':
        case 'bypassed':
        case 'clean':
...
function mapManualConversionRow(row) {
...
function addCommonClickFilters(values, conditions, input, scope) {
    ...
    if (input.networkAccountId !== undefined) {
        conditions.push(`click.network_account_id = ${appendValue(values, input.networkAccountId)}::uuid`);
    }
    if (input.ownerMembershipId !== undefined) {
        conditions.push(`click.owner_membership_id = ${appendValue(values, input.ownerMembershipId)}::uuid`);
    }
    ...
    if (input.countryCode !== undefined) {
        conditions.push(`${countryCodeExpression} = ${appendValue(values, input.countryCode.toUpperCase())}`);
    }
...
function addManagerConversionScope(values, conditions, scope) {
    if (scope.managerMembershipId === undefined) {
        return;
    }
    const managerPlaceholder = appendValue(values, scope.managerMembershipId);
...
function buildDimensionQuery(dimension) {
    switch (dimension) {
        ...
        `,
                dimensionId: 'dimension.id',
                dimensionName: "coalesce(dimension_profile.display_name, dimension_user.email, 'Unknown Manager')",
                dimensionStatus: 'dimension.status::text',
                clickJoin: `
          exists (
            select 1
        ...
        `,
                dimensionId: 'dimension.id',
                dimensionName: "coalesce(dimension_profile.display_name, dimension_user.email, 'Unknown publisher')",
                dimensionStatus: 'dimension.status::text',
...
export function createFinalOperationsRepository(database) {
    return Object.freeze({
        async listPerformanceReport(context, companyId, dimension, input, scope) {
            ...
            return database.transaction(async (transaction) => {
                ...
                addCommonClickFilters(values, clickConditions, {
                ...
                if (input.from !== undefined) {
                    conversionConditions.push(`conversion.converted_at >= ${appendValue(values, input.from)}::timestamptz`);
                }
                if (input.to !== undefined) {
                    conversionConditions.push(`conversion.converted_at < ${appendValue(values, input.to)}::timestamptz`);
                }
                if (input.offerId !== undefined) {
                    conversionConditions.push(`conversion.offer_id = ${appendValue(values, input.offerId)}::uuid`);
                }
                if (input.networkAccountId !== undefined) {
                    conversionConditions.push(`conversion.network_account_id = ${appendValue(values, input.networkAccountId)}::uuid`);
                }
                if (input.ownerMembershipId !== undefined) {
                    conversionConditions.push(`conversion.owner_membership_id = ${appendValue(values, input.ownerMembershipId)}::uuid`);
                }
                if (scope.ownerUserId !== undefined) {
                    conversionConditions.push(`conversion.owner_user_id = ${appendValue(values, scope.ownerUserId)}::uuid`);
                }
                addManagerConversionScope(values, conversionConditions, scope);
                if (input.countryCode !== undefined) {
                    conversionConditions.push(`${countryCodeExpression} = ${appendValue(values, input.countryCode.toUpperCase())}`);
                }
                if (input.device !== undefined) {
                    conversionConditions.push(`${deviceExpression} = ${appendValue(values, input.device)}`);
                }
                const searchPlaceholder = input.search === undefined ? null : appendValue(values, `%${input.search}%`);
                const statusPlaceholder = input.status === undefined ? null : appendValue(values, input.status);
                const publisherScopePlaceholder = dimension === 'publishers' && scope.ownerUserId !== undefined
                    ? appendValue(values, scope.ownerUserId)
                    : null;
                const limitPlaceholder = appendValue(values, input.limit);
                const managerDimensionCondition = scope.managerMembershipId === undefined
                    ? null
                    : (() => {
                        const managerPlaceholder = appendValue(values, scope.managerMembershipId);
                        switch (dimension) {
        ...
        async listConversions(context, companyId, input, scope) {
            return database.transaction(async (transaction) => {
                ...
                if (input.from !== undefined) {
                    conditions.push(`conversion.converted_at >= ${appendValue(values, input.from)}::timestamptz`);
                }
                if (input.to !== undefined) {
                    conditions.push(`conversion.converted_at < ${appendValue(values, input.to)}::timestamptz`);
                }
                if (input.offerId !== undefined) {
                    conditions.push(`conversion.offer_id = ${appendValue(values, input.offerId)}::uuid`);
                }
                if (input.networkAccountId !== undefined) {
                    conditions.push(`conversion.network_account_id = ${appendValue(values, input.networkAccountId)}::uuid`);
                }
                if (input.ownerMembershipId !== undefined) {
                    conditions.push(`conversion.owner_membership_id = ${appendValue(values, input.ownerMembershipId)}::uuid`);
                }
                if (scope.ownerUserId !== undefined) {
                    conditions.push(`conversion.owner_user_id = ${appendValue(values, scope.ownerUserId)}::uuid`);
                }
                addManagerConversionScope(values, conditions, scope);
                if (input.conversionStatus !== undefined) {
                    conditions.push(`conversion.status = ${appendValue(values, input.conversionStatus)}::public.conversion_status`);
                }
                if (input.status !== undefined) {
                    conditions.push(`${conversionReviewStatusExpression} = ${appendValue(values, input.status)}`);
                }
                if (input.countryCode !== undefined) {
                    conditions.push(`${countryCodeExpression} = ${appendValue(values, input.countryCode.toUpperCase())}`);
                }
        ...
        async listSessions(context, companyId, input, scope) {
            return database.transaction(async (transaction) => {
                const values = [companyId];
                const conditions = ['click.company_id = $1'];
                addCommonClickFilters(values, conditions, {
                    ...input,
                }, scope);
                const limitPlaceholder = appendValue(values, input.limit);
                const result = await transaction.query({
        ...
        async createManualConversion(context, companyId, input, scope) {
            return database.transaction(async (transaction) => {
                const clickResult = await transaction.query({
            ...
            `,
                ...
                if (click?.payout_amount_minor === undefined ||
                    click.payout_amount_minor === null ||
                    click.payout_currency === null) {
                    return undefined;
                }
                const payoutAmountMinor = normalizeSafeInteger(click.payout_amount_minor, 'payout_amount_minor');
...
//# sourceMappingURL=final-operations.repository.js.map