...
function parseNetworkStatus(value) {
    switch (value) {
        case 'active':
        case 'suspended':
        case 'archived':
...
function parseRedirectType(value) {
    switch (value) {
        case '301':
        case '302':
            return value;
...
function parseReferrerMode(value) {
    switch (value) {
        case 'preserve':
        case 'strip':
            return value;
...
function mapPublisher(row) {
    ...
    return Object.freeze({
        ...
        fixedPayoutAmountMinor: row.fixed_payout_amount_minor === null ? null : normalizeCount(row.fixed_payout_amount_minor),
        payoutCurrency: row.payout_currency,
        postbackUrl: row.postback_url,
...
`;
async function writeAuditEvent(transaction, context, input) {
...
async function loadOffer(transaction, companyId, offerId) {
...
async function loadNetwork(transaction, companyId, accountId) {
    const result = await transaction.query({
...
async function loadPublisher(transaction, companyId, membershipId) {
    const result = await transaction.query({
...
async function replaceManagerOfferAssignments(transaction, context, offerId, membershipIds) {
    await transaction.query({
...
export function createCatalogOperationsRepository(database) {
    return Object.freeze({
        ...
        async getSnapshot(context, companyId) {
            ...
            `,
                ...
                const domainResult = await transaction.query({
            ...
            `,
                ...
                const networkResult = await transaction.query({
            ...
            `,
                ...
                const offerResult = await transaction.query({
            ...
            `,
                ...
                const managerResult = await transaction.query({
            ...
            `,
                ...
                const publisherResult = await transaction.query({
        ...
        async createOffer(context, companyId, input) {
            ...
            `,
                ...
                await replaceManagerOfferAssignments(transaction, context, offerId, input.managerMembershipIds);
            ...
            `,
              ...
              `,
                    ...
                    if (assignmentResult.rows.length !== input.assignedOfferIds.length) {
                        throw new Error('One or more selected Offers are unavailable or missing default payout configuration.');
                    }
                }
                await writeAuditEvent(transaction, context, {
...
//# sourceMappingURL=catalog-operations.repository.js.map