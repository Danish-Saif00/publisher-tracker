...
function parseRuleStatus(value) {
    switch (value) {
        case 'active':
        case 'paused':
        case 'archived':
...
function parseDuplicateDecision(value) {
    switch (value) {
        case 'accepted':
        case 'duplicate':
            return value;
...
function parseFraudRiskLevel(value) {
    switch (value) {
        case 'low':
        case 'medium':
        case 'high':
...
function normalizeFraudSignals(value) {
...
`;
export function createDuplicateFraudRepository(database) {
    return Object.freeze({
        ...
        async listRules(context, companyId, query) {
            return database.transaction(async (transaction) => {
                ...
                const conditions = ['rule.company_id = $1'];
                if (query.networkAccountId !== undefined) {
                    conditions.push(`rule.network_account_id = ${appendQueryValue(values, query.networkAccountId)}::uuid`);
                }
                if (query.offerId !== undefined) {
                    conditions.push(`rule.offer_id = ${appendQueryValue(values, query.offerId)}::uuid`);
                }
                if (query.status !== undefined) {
                    conditions.push(`rule.status = ${appendQueryValue(values, query.status)}::public.duplicate_protection_rule_status`);
                }
                const result = await transaction.query({
        ...
        async listFraudClicks(context, companyId, query) {
            return database.transaction(async (transaction) => {
                ...
                if (query.networkAccountId !== undefined) {
                    conditions.push(`click.network_account_id = ${appendQueryValue(values, query.networkAccountId)}::uuid`);
                }
                if (query.offerId !== undefined) {
                    conditions.push(`click.offer_id = ${appendQueryValue(values, query.offerId)}::uuid`);
                }
                if (query.duplicateDecision !== undefined) {
                    conditions.push(`click.duplicate_decision = ${appendQueryValue(values, query.duplicateDecision)}::public.duplicate_decision`);
                }
                if (query.fraudRiskLevel !== undefined) {
                    conditions.push(`click.fraud_risk_level = ${appendQueryValue(values, query.fraudRiskLevel)}::public.fraud_risk_level`);
                }
                if (query.visibleToUserId !== undefined) {
                    conditions.push(`click.owner_user_id = ${appendQueryValue(values, query.visibleToUserId)}::uuid`);
                }
...
//# sourceMappingURL=duplicate-fraud.repository.js.map