...
const LINK_IDENTIFIER_MODES =
  new Set([
    'slug_or_code',
    'tracking_code',
  ]);
const RESTRICTED_SHARE_PLATFORMS =
  new Set([
    'snapchat',
    'instagram',
    'facebook',
  ]);
const QUERY_PARAMETER_KEY_PATTERN =
  /^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$/u;
function normalizeLinkIdentifierMode(
  value: string,
):
  | 'slug_or_code'
  | 'tracking_code' {
...
function normalizeRestrictedSharePlatforms(
  ...
  for (const platform of normalized) {
    if (
      !RESTRICTED_SHARE_PLATFORMS.has(
        platform,
      )
    ) {
      throw new ApiHttpError(
...
function normalizeDefaultLinkQueryParameters(
  ...
  for (
    ...
    const parameterValue =
      rawValue.trim();
    if (
      !QUERY_PARAMETER_KEY_PATTERN.test(
        key,
      )
    ) {
      throw new ApiHttpError(
...
function toPublicProxyConfiguration(
  ...
  return Object.freeze({
    ...
    bypassOwnerMembershipIds:
      record.bypassOwnerMembershipIds,
    apiKeyUpdatedAt:
      record.apiKeyUpdatedAt,
    lastTestedAt:
      record.lastTestedAt,
    lastTestStatus:
      record.lastTestStatus,
    lastTestErrorCode:
      record.lastTestErrorCode,
...
function normalizeProxyProviderCode(
  value: string,
): CompanyProxyProviderCode {
  if (
...
function normalizeProxyInteger(
  ...
  if (
    ...
    throw new ApiHttpError(
      'INVALID_REQUEST_BODY',
      400,
      `${fieldName} must be a whole number between ${String(
        minimum,
      )} and ${String(maximum)}.`,
    );
  }
...
function normalizeProxyBypassMembershipIds(
...
export function createCompanyOperationsService(
  ...
  return Object.freeze<CompanyOperationsService>({
      ...
      return repository.upsertCustomization(context, companyId, {
        ...
        linkIdentifierMode:
          input.linkIdentifierMode === undefined
            ? (
                current?.linkIdentifierMode ??
                'slug_or_code'
              )
            : normalizeLinkIdentifierMode(
                input.linkIdentifierMode,
              ),
        plainTextSharingEnabled:
          input.plainTextSharingEnabled ??
          current?.plainTextSharingEnabled ??
          true,
        restrictedSharePlatforms:
          ...
          undefined
            ...
            : normalizeRestrictedSharePlatforms(
                input.restrictedSharePlatforms,
              ),
        defaultLinkQueryParameters:
          ...
          undefined
            ...
            : normalizeDefaultLinkQueryParameters(
                input.defaultLinkQueryParameters,
              ),
      });
    },
    ...
    async getProxyConfiguration(
      ...
      assertConfigurationWriteAccess(
        identity,
        companyId,
      );
      const context =
      ...
      return configuration === undefined
        ? null
        : toPublicProxyConfiguration(
            configuration,
          );
    },
    async updateProxyConfiguration(
      identity,
      requestId,
      companyIdValue,
      input,
      ...
      const bypassOwnerMembershipIds =
        normalizeProxyBypassMembershipIds(
          input
            .bypassOwnerMembershipIds,
        );
      const validBypassCount =
        await repository
      ...
      if (
        validBypassCount !==
        bypassOwnerMembershipIds.length
      ) {
        throw new ApiHttpError(
      ...
      return toPublicProxyConfiguration(
        configuration,
      );
    },
