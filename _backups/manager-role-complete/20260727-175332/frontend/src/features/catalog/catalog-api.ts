...
function readStringArray(value: unknown, fieldName: string): readonly string[] {
  return readArray(value, fieldName).map((item) => readRequiredString(item, fieldName));
}

function readNumberArray(value: unknown, fieldName: string): readonly number[] {
  return readArray(value, fieldName).map((item) => readRequiredNumber(item, fieldName));
}
...
function parseDomain(value: unknown): CatalogDomain {
  ...
  return {
    ...
    verifiedAt: readNullableString(value.verifiedAt, 'domain verification time'),
...
function parseNetwork(value: unknown): CatalogNetwork {
  ...
  return {
    ...
    providerCode: readRequiredString(value.providerCode, 'network provider code'),
    providerName: readRequiredString(value.providerName, 'network provider name'),
    name: readRequiredString(value.name, 'network name'),
    externalAccountId: readNullableString(value.externalAccountId, 'external account id'),
    status: readStatus<CatalogNetworkStatus>(value.status, 'network status', [
      'active',
      'suspended',
      'archived',
    ]),
    trackingParameter: readNullableString(value.trackingParameter, 'tracking parameter'),
    postbackUrl: readNullableString(value.postbackUrl, 'postback URL'),
    duplicateAllowed: readBoolean(value.duplicateAllowed, 'network duplicate flag'),
...
function parseOffer(value: unknown): CatalogOffer {
  ...
  const devices = readStringArray(value.devices, 'offer devices').map((item) =>
    readStatus<CatalogDevice>(item, 'offer device', ['desktop', 'android', 'ios']),
  );
  ...
  return {
    ...
    networkAccountId: readRequiredString(value.networkAccountId, 'offer network id'),
    networkAccountName: readRequiredString(value.networkAccountName, 'offer network name'),
    ...
    trackingDomainId: readNullableString(value.trackingDomainId, 'offer domain id'),
    trackingDomainHostname: readNullableString(value.trackingDomainHostname, 'offer domain hostname'),
    ...
    redirectType: readStatus<CatalogRedirectType>(value.redirectType, 'redirect type', ['301', '302']),
    referrerMode: readStatus<CatalogReferrerMode>(value.referrerMode, 'referrer mode', ['preserve', 'strip']),
    defaultPayoutAmountMinor: readNullableNumber(value.defaultPayoutAmountMinor, 'default payout'),
    ...
    activeStartTime: readNullableString(value.activeStartTime, 'offer start time'),
    ...
    duplicateAllowed: readBoolean(value.duplicateAllowed, 'offer duplicate flag'),
    managerMembershipIds: readStringArray(value.managerMembershipIds, 'offer manager ids'),
    publisherMembershipIds: readStringArray(value.publisherMembershipIds, 'offer publisher ids'),
    clicks: readRequiredNumber(value.clicks, 'offer click count'),
    conversions: readRequiredNumber(value.conversions, 'offer conversion count'),
...
function parseManager(value: unknown): CatalogManager {
  ...
  return {
    membershipId: readRequiredString(value.membershipId, 'Manager membership id'),
    ...
    userStatus: readStatus(value.userStatus, 'Manager user status', ['active', 'suspended']),
    membershipStatus: readStatus(value.membershipStatus, 'Manager membership status', [
      'invited',
      'active',
      'suspended',
      'revoked',
...
function parsePublisher(value: unknown): CatalogPublisher {
  ...
  return {
    membershipId: readRequiredString(value.membershipId, 'publisher membership id'),
    ...
    userStatus: readStatus(value.userStatus, 'publisher user status', ['active', 'suspended']),
    membershipStatus: readStatus(value.membershipStatus, 'publisher membership status', [
      'invited',
      'active',
      'suspended',
      'revoked',
    ...
    payoutType: readStatus<CatalogPayoutType>(value.payoutType, 'publisher payout type', [
      'fixed_member',
      'per_offer',
    ]),
    postbackUrl: readNullableString(value.postbackUrl, 'publisher postback URL'),
    emailNotificationsEnabled: readBoolean(value.emailNotificationsEnabled, 'publisher email flag'),
...
function parseSnapshot(value: unknown): CoreCatalogSnapshot {
  ...
  return {
    ...
    summary: {
      ...
      publishers: readRequiredNumber(value.summary.publishers, 'publisher count'),
    },
