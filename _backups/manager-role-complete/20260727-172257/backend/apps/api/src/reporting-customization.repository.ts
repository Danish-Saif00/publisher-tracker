...
function mapCustomizationRow(row: CustomizationRow): CompanyCustomizationRecord {
  return Object.freeze({
    ...
    linkIdentifierMode:
      row.link_identifier_mode as
        CompanyCustomizationRecord['linkIdentifierMode'],
    plainTextSharingEnabled:
      row.plain_text_sharing_enabled,
    restrictedSharePlatforms:
      row.restricted_share_platforms as
        CompanyCustomizationRecord['restrictedSharePlatforms'],
    defaultLinkQueryParameters:
      row.default_link_query_parameters,
...
function mapProxyProviderCode(
  value: string,
): CompanyProxyConfigurationRecord['providerCode'] {
  if (
...
function mapProxyStatus(
  value: string,
): CompanyProxyConfigurationRecord['status'] {
  if (
    value === 'active' ||
    value === 'disabled'
  ) {
  ...
  throw new Error(
    'The database returned an unsupported proxy status.',
  );
}
...
function mapProxyFailureBehavior(
  ...
  throw new Error(
    'The database returned an unsupported proxy failure behavior.',
  );
}
function mapProxyTestStatus(
  ...
  throw new Error(
    'The database returned an unsupported proxy test status.',
  );
}
function mapProxyRow(
  row: ProxyRow,
): CompanyProxySecretRecord {
  return Object.freeze({
    id: row.id,
    ...
    apiKeyLast4:
      row.api_key_last4,
    hasApiKey:
      row.encrypted_api_key.length > 0,
    status:
      mapProxyStatus(row.status),
    enforcementMode:
      mapProxyEnforcementMode(
  ...
  return Object.freeze<CompanyOperationsRepository>({
        ...
        async (transaction) => {
          const result = await transaction.query<CustomizationRow>({
            ...
            values: [
              ...
              input.linkIdentifierMode ??
                'slug_or_code',
              input.plainTextSharingEnabled ??
                true,
              [
                ...(
                  input.restrictedSharePlatforms ??
                  [
                    'snapchat',
                    'instagram',
                    'facebook',
              ...
              JSON.stringify(
                input.defaultLinkQueryParameters ??
                  {},
              ),
              context.actorUserId,
            ],
          ...
          await writeAuditEvent(transaction, {
            ...
            metadata: {
              ...
              plainTextSharingEnabled:
                customization.plainTextSharingEnabled,
              restrictedSharePlatforms:
                customization.restrictedSharePlatforms,
              defaultLinkQueryParameterCount:
                Object.keys(
                  customization.defaultLinkQueryParameters,
                ).length,
            },
          });
    ...
    async getProxyConfiguration(
      ...
      return database.transaction(
        async (transaction) => {
          const result =
            await transaction.query<ProxyRow>({
              name:
                'company-operations-get-proxy-configuration',
              text: `
        ...
        },
        {
          readOnly: true,
          sessionContext:
            createDatabaseSessionContext(
              context,
            ),
        },
      );
    },
    async countValidProxyBypassMemberships(
      context,
      companyId,
      membershipIds,
    ) {
      ...
      return database.transaction(
        async (transaction) => {
          const result =
            await transaction.query<ProxyBypassCountRow>({
              name:
                'company-operations-count-proxy-bypass-memberships',
              text: `
                select
    ...
    async upsertProxyConfiguration(
      context,
      companyId,
      input,
    ) {
      return database.transaction(
        async (transaction) => {
          const result =
            await transaction.query<ProxyRow>({
              name:
                'company-operations-upsert-proxy-configuration',
              text: `
          ...
          if (row === undefined) {
            throw new Error(
              'The proxy configuration could not be saved.',
            );
          }
          const configuration =
            mapProxyRow(row);
          await writeAuditEvent(
            transaction,
            {
              ...
              requestId:
                context.requestId,
              eventName:
                'company_proxy_configuration.updated',
              entityType:
                'company_proxy_configuration',
              entityId:
                configuration.id,
              metadata: {
                providerCode:
