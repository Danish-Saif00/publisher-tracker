...
function normalizeLimit(value, fallback = 200) {
    ...
    if (!Number.isInteger(normalized) || normalized < 1 || normalized > MAX_LIMIT) {
        throw new ApiHttpError('INVALID_QUERY_PARAMETER', 400, `limit must be a whole number between 1 and ${String(MAX_LIMIT)}.`);
    }
    return normalized;
}
function normalizeTimestamp(value, fieldName) {
    ...
    if (Number.isNaN(timestamp.getTime())) {
        throw new ApiHttpError('INVALID_QUERY_PARAMETER', 400, `${fieldName} must be a valid date and time.`);
    }
...
function normalizeRange(input) {
    ...
    if (from !== undefined &&
        ...
        throw new ApiHttpError('INVALID_QUERY_PARAMETER', 400, 'The selected range cannot exceed 366 days.');
    }
...
function normalizeCountryCode(value) {
    ...
    if (!COUNTRY_CODE_PATTERN.test(normalized)) {
        throw new ApiHttpError('INVALID_QUERY_PARAMETER', 400, 'countryCode must use a two-letter country code.');
    }
    return normalized;
}
function normalizeDevice(value) {
    ...
    switch (value) {
        ...
        case 'other':
            return value;
        default:
            throw new ApiHttpError('INVALID_QUERY_PARAMETER', 400, 'device must be desktop, mobile, tablet, or other.');
    }
}
function normalizeReviewStatus(value) {
    ...
    switch (value) {
        ...
        case 'unchecked':
            return value;
        default:
            throw new ApiHttpError('INVALID_QUERY_PARAMETER', 400, 'status must be approved, rejected, or unchecked.');
    }
}
...
function normalizeDimensionStatus(value) {
    if (value === undefined || value.trim().length === 0) {
        return undefined;
    }
...
function normalizeReportDimension(value) {
    switch (value) {
        ...
        case 'publishers':
            return value;
        default:
            throw new ApiHttpError('INVALID_PATH_PARAMETER', 400, 'dimension must be networks, offers, managers, or publishers.');
    }
}
function createContext(identity, requestId, companyId) {
    return Object.freeze({
...
function assertPublisherReportDimension(identity, dimension) {
    if (identity.companyMembership?.role === 'publisher' && dimension !== 'offers') {
        throw new ApiHttpError('PUBLISHER_REPORT_DIMENSION_FORBIDDEN', 403, 'Publishers can access only their own Offer performance report.');
    }
}
function assertPublisherOwnerFilter(identity, ownerMembershipId) {
    ...
    if (membership?.role === 'publisher' &&
        ...
        throw new ApiHttpError('PUBLISHER_OWNER_FILTER_FORBIDDEN', 403, 'Publishers cannot request logs or reports for another membership.');
    }
}
function assertReadAccess(identity, companyId) {
...
function assertBillingAccess(identity, companyId) {
...
function normalizeDisplayName(value) {
    ...
    if (normalized.length > 120) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'displayName cannot exceed 120 characters.');
    }
    return normalized;
}
function normalizeTimezone(value) {
    ...
    if (normalized.length < 1 || normalized.length > 64) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'timezone must contain 1 to 64 characters.');
    }
    try {
...
function normalizeManualConversionInput(input) {
    const publicClickId = input.publicClickId.trim().toLowerCase();
    if (!PUBLIC_CLICK_ID_PATTERN.test(publicClickId)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'publicClickId must be a valid Publisher Tracker click ID.');
    }
    let status;
    switch (input.status) {
        case 'pending':
        ...
        case 'rejected':
            ...
            break;
        default:
            throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'status must be pending, approved, or rejected.');
    }
    const revenueAmountMinor = input.revenueAmountMinor ?? null;
    const revenueCurrency = input.revenueCurrency === undefined || input.revenueCurrency === null
        ? null
        : input.revenueCurrency.trim().toUpperCase();
    if ((revenueAmountMinor === null) !== (revenueCurrency === null)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'revenueAmountMinor and revenueCurrency must be provided together.');
    }
    if (revenueAmountMinor !== null &&
        (!Number.isSafeInteger(revenueAmountMinor) || revenueAmountMinor < 0)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'revenueAmountMinor must be a non-negative whole number.');
    }
    if (revenueCurrency !== null && !CURRENCY_PATTERN.test(revenueCurrency)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'revenueCurrency must be a three-letter currency code.');
    }
    return Object.freeze({
        publicClickId,
        status,
...
export function createFinalOperationsService(repository) {
    return Object.freeze({
        async listPerformanceReport(identity, requestId, companyIdValue, dimensionValue, input) {
            ...
            return repository.listPerformanceReport(createContext(identity, requestId, companyId), companyId, dimension, {
                ...filters,
        ...
        async listClicks(identity, requestId, companyIdValue, input) {
            ...
            assertReadAccess(identity, companyId);
            assertPublisherOwnerFilter(identity, filters.ownerMembershipId);
            return repository.listClicks(createContext(identity, requestId, companyId), companyId, {
                ...filters,
                ...(status !== undefined ? { status } : {}),
        ...
        async listConversions(identity, requestId, companyIdValue, input) {
            ...
            assertReadAccess(identity, companyId);
            assertPublisherOwnerFilter(identity, filters.ownerMembershipId);
            return repository.listConversions(createContext(identity, requestId, companyId), companyId, {
                ...filters,
                ...(status !== undefined ? { status } : {}),
        ...
        async listSessions(identity, requestId, companyIdValue, input) {
            ...
            assertReadAccess(identity, companyId);
            assertPublisherOwnerFilter(identity, filters.ownerMembershipId);
            return repository.listSessions(createContext(identity, requestId, companyId), companyId, {
                ...filters,
                limit: normalizeLimit(input.limit),
            }, createScope(identity));
        },
        async listUserAgents(identity, requestId, companyIdValue, input) {
            ...
            assertReadAccess(identity, companyId);
            assertPublisherOwnerFilter(identity, filters.ownerMembershipId);
            return repository.listUserAgents(createContext(identity, requestId, companyId), companyId, {
                ...filters,
                ...(status !== undefined ? { status } : {}),
                limit: normalizeLimit(input.limit),
            }, createScope(identity));
        ...
        async getAccountProfile(identity, requestId) {
            const profile = await repository.getAccountProfile(createContext(identity, requestId), identity.actor.userId);
            if (profile === undefined) {
                throw new ApiHttpError('ACCOUNT_PROFILE_NOT_FOUND', 404, 'The account profile was not found.');
            }
            return profile;
        },
        async updateAccountProfile(identity, requestId, input) {
            const profile = await repository.updateAccountProfile(createContext(identity, requestId), identity.actor.userId, {
        ...
        async listBillingInvoices(identity, requestId, companyIdValue, limit) {
            ...
            return repository.listBillingInvoices(createContext(identity, requestId, companyId), companyId, normalizeLimit(limit, 100));
        },
        async createManualConversion(identity, requestId, companyIdValue, input) {
            const companyId = normalizeUuid(companyIdValue, 'companyId');
            assertOperationsAccess(identity, companyId);
            const conversion = await repository.createManualConversion(createContext(identity, requestId, companyId), companyId, normalizeManualConversionInput(input), createScope(identity));
            if (conversion === undefined) {
                throw new ApiHttpError('MANUAL_CONVERSION_CONFLICT', 409, 'The click is unavailable, ineligible, missing payout configuration, or already has a manual conversion.');
            }
            return conversion;
        },
    });
}
//# sourceMappingURL=final-operations.service.js.map