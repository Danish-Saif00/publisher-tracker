...
function isPublisher(identity, companyId) {
    const membership = identity.companyMembership;
    return (membership?.companyId === companyId &&
        membership.status === 'active' &&
        membership.role === 'publisher');
}
function assertPublisherTrackingDomain(identity, companyId, offer, trackingDomainId) {
    if (!isPublisher(identity, companyId)) {
        return;
    }
    if (offer.trackingDomainId === null || offer.trackingDomainId !== trackingDomainId) {
        throw new ApiHttpError('TRACKING_LINK_DOMAIN_FORBIDDEN', 403, 'Publishers can use only the active tracking domain configured for their assigned Offer.');
    }
}
function assertPublisherDestinationOverride(identity, companyId, requestedDestinationUrl) {
    if (!isPublisher(identity, companyId) || requestedDestinationUrl === undefined) {
        return;
    }
    throw new ApiHttpError('TRACKING_LINK_DESTINATION_OVERRIDE_FORBIDDEN', 403, 'Publishers cannot override an assigned Offer destination URL.');
}
...
export function createTrackingLinksService(repository) {
    return Object.freeze({
        ...
        async updateTrackingLink(identity, requestId, companyIdValue, linkIdValue, input) {
            ...
            const context = createRepositoryContext(identity, requestId, companyId);
            await requireActiveCompany(repository, context, companyId);
            const current = await repository.getTrackingLink(context, companyId, linkId, resolveVisibleToUserId(identity));
            ...
            if (isPublisher(identity, companyId)) {
                if (input.trackingDomainId !== undefined && trackingDomainId !== current.trackingDomainId) {
                    throw new ApiHttpError('TRACKING_LINK_DOMAIN_FORBIDDEN', 403, 'Publishers cannot move a tracking link to another tracking domain.');
                }
            ...
            if (isPublisher(identity, companyId) ||
                ...
                const dependencies = await requireActivationDependencies(repository, context, companyId, current.offerId, trackingDomainId, current.ownerMembershipId);
                assertPublisherTrackingDomain(identity, companyId, dependencies.offer, trackingDomainId);
                if (isPublisher(identity, companyId) &&
                    destinationUrl !== normalizeDestinationUrl(dependencies.offer.destinationUrl)) {
                    throw new ApiHttpError('TRACKING_LINK_DESTINATION_OVERRIDE_FORBIDDEN', 403, 'Publishers can use only the destination URL configured for their assigned Offer.');
                }
            }
