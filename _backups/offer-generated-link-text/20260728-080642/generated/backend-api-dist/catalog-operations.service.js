...
function normalizeNullableText(value, fieldName, maximumLength) {
    ...
    if (normalized.length > maximumLength) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, `${fieldName} must contain at most ${String(maximumLength)} characters.`);
    }
    return normalized;
}
function normalizeUrl(value, fieldName, required) {
...
function normalizeOfferCode(value) {
    ...
    if (normalized.length < 2 || normalized.length > 80 || !OFFER_CODE_PATTERN.test(normalized)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'code must contain 2 to 80 lowercase letters, numbers, underscores, or hyphens.');
    }
    return normalized;
}
function normalizeCountries(values) {
    ...
    if (normalized.some((value) => !COUNTRY_PATTERN.test(value))) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'countries must contain two-letter uppercase country codes.');
    }
...
function normalizeActiveDays(values) {
...
function normalizePayout(amount, currency) {
    ...
    if (normalizedAmount === null ||
        ...
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'defaultPayoutAmountMinor and payoutCurrency must be provided together using a positive integer and three-letter currency code.');
    }
...
function normalizeManagerMembershipIds(values) {
    const normalized = values.map((value) => normalizeUuid(value, 'Manager membership ID'));
    if (new Set(normalized).size !== normalized.length) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'managerMembershipIds cannot contain duplicate values.');
    }
    return Object.freeze(normalized);
}
function normalizeAssignedOfferIds(values) {
    const normalized = values.map((value) => normalizeUuid(value, 'Assigned Offer ID'));
    if (new Set(normalized).size !== normalized.length) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'assignedOfferIds cannot contain duplicate values.');
    }
    return Object.freeze(normalized);
}
function normalizePublisherPayout(payoutType, amount, currency) {
    if (payoutType === 'per_offer') {
        if ((amount !== undefined && amount !== null) ||
            (currency !== undefined && currency !== null)) {
            throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'per_offer payout cannot define a fixed Publisher payout.');
        }
        return Object.freeze({ amount: null, currency: null });
    }
    const normalizedAmount = amount ?? null;
    ...
    if (normalizedAmount === null ||
        ...
        normalizedAmount > MAX_AMOUNT_MINOR ||
        !CURRENCY_PATTERN.test(normalizedCurrency)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'fixed_member payout requires a positive fixedPayoutAmountMinor and a three-letter payoutCurrency.');
    }
...
function hidePublisherCatalogDetails(snapshot) {
    return Object.freeze({
        ...snapshot,
...
function createPublisherOfferDirectory(snapshot, membershipId) {
    const activeDomainIds = new Set(snapshot.domains.filter((domain) => domain.status === 'active').map((domain) => domain.id));
    return Object.freeze(snapshot.offers
        .filter((offer) => offer.status === 'active' && offer.publisherMembershipIds.includes(membershipId))
        .map((offer) => {
        const hasActiveTrackingDomain = offer.trackingDomainId !== null && activeDomainIds.has(offer.trackingDomainId);
        return Object.freeze({
            id: offer.id,
...
function scopeCatalogSnapshotForManager(snapshot, membershipId, actorUserId) {
    const offers = Object.freeze(snapshot.offers.filter((offer) => offer.managerMembershipIds.includes(membershipId)));
    const offerIds = new Set(offers.map((offer) => offer.id));
    const networkIds = new Set(offers.map((offer) => offer.networkAccountId));
    const domainIds = new Set(offers.flatMap((offer) => (offer.trackingDomainId === null ? [] : [offer.trackingDomainId])));
    const providerIds = new Set(offers.map((offer) => offer.providerId));
    const networks = Object.freeze(snapshot.networks
        .filter((network) => networkIds.has(network.id))
        .map((network) => Object.freeze({
        ...network,
        offerCount: offers.filter((offer) => offer.networkAccountId === network.id && offer.status !== 'archived').length,
    })));
    const domains = Object.freeze(snapshot.domains
        .filter((domain) => domainIds.has(domain.id))
        .map((domain) => Object.freeze({
        ...domain,
        offerCount: offers.filter((offer) => offer.trackingDomainId === domain.id && offer.status !== 'archived').length,
    })));
    const providers = Object.freeze(snapshot.providers.filter((provider) => providerIds.has(provider.id)));
    const managers = Object.freeze(snapshot.managers
        .filter((manager) => manager.membershipId === membershipId)
        .map((manager) => Object.freeze({
        ...manager,
        offerCount: offers.filter((offer) => offer.status !== 'archived').length,
    })));
    const publishers = Object.freeze(snapshot.publishers
        .filter((publisher) => publisher.invitedBy === actorUserId ||
        publisher.managerMembershipIds.includes(membershipId))
        .map((publisher) => {
        const assignedOfferIds = Object.freeze(publisher.assignedOfferIds.filter((offerId) => offerIds.has(offerId)));
        return Object.freeze({
            ...publisher,
            offerCount: assignedOfferIds.length,
            assignedOfferIds,
            managerMembershipIds: Object.freeze(publisher.managerMembershipIds.filter((managerMembershipId) => managerMembershipId === membershipId)),
        });
    }));
    return Object.freeze({
...
async function requireActiveCompany(repository, context, companyId) {
    ...
    if (company.status !== 'active') {
        throw new ApiHttpError('CATALOG_COMPANY_INACTIVE', 409, 'Catalog operations require an active company.');
    }
}
function normalizeOfferInput(input, networkAccountId, code, status) {
    ...
    if ((activeStartTime === null) !== (activeEndTime === null)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'activeStartTime and activeEndTime must be provided together.');
    }
    if (activeStartTime !== null && activeEndTime !== null && activeStartTime >= activeEndTime) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'activeStartTime must be earlier than activeEndTime.');
    }
...
function normalizeTrackingParameter(value) {
    ...
    if (normalized !== null && !TRACKING_PARAMETER_PATTERN.test(normalized)) {
        throw new ApiHttpError('INVALID_REQUEST_BODY', 400, 'trackingParameter may only contain letters, numbers, dots, underscores, or hyphens.');
    }
    return normalized;
}
function normalizeNetworkInput(input, providerId, status) {
    return Object.freeze({
        providerId,
        name: normalizeRequiredText(input.name, 'name', 2, 160),
...
function validateOfferReferences(snapshot, input) {
    const account = snapshot.networks.find((item) => item.id === input.networkAccountId);
    if (account?.status !== 'active') {
        throw new ApiHttpError('CATALOG_NETWORK_INVALID', 409, 'The offer requires an active network from the selected company.');
    }
    const domain = snapshot.domains.find((item) => item.id === input.trackingDomainId);
    if (domain?.status !== 'active') {
        throw new ApiHttpError('CATALOG_DOMAIN_INVALID', 409, 'The offer requires an active verified tracking domain.');
    }
    const eligibleManagerIds = new Set(snapshot.managers
    ...
    if (input.managerMembershipIds.some((membershipId) => !eligibleManagerIds.has(membershipId))) {
        throw new ApiHttpError('CATALOG_MANAGER_INVALID', 409, 'Offers can only be assigned to active Manager memberships.');
    }
}
export function createCatalogOperationsService(repository) {
    return Object.freeze({
        async getSnapshot(identity, requestId, companyIdValue) {
            ...
            if (identity.companyMembership?.role === 'manager') {
                return scopeCatalogSnapshotForManager(snapshot, identity.companyMembership.membershipId, identity.actor.userId);
            }
        ...
        async listPublisherOffers(identity, requestId, companyIdValue) {
            ...
            if (membershipId === undefined) {
                throw new ApiHttpError('CATALOG_PUBLISHER_INVALID', 403, 'An active Publisher membership is required.');
            }
            const context = createRepositoryContext(identity, requestId, companyId);
            await requireActiveCompany(repository, context, companyId);
            return createPublisherOfferDirectory(await repository.getSnapshot(context, companyId), membershipId);
        },
        async createOffer(identity, requestId, companyIdValue, input) {
            ...
            await requireActiveCompany(repository, context, companyId);
            const snapshot = await repository.getSnapshot(context, companyId);
            const normalized = normalizeOfferInput(input, normalizeUuid(input.networkAccountId, 'Network account ID'), normalizeOfferCode(input.code), input.status === undefined ? 'draft' : normalizeOfferStatus(input.status));
            validateOfferReferences(snapshot, normalized);
            const created = await repository.createOffer(context, companyId, normalized);
            if (created === undefined) {
                throw new ApiHttpError('CATALOG_OFFER_CONFLICT', 409, 'An offer with this code or external network identifier already exists.');
            }
            return created;
        },
        async updateOffer(identity, requestId, companyIdValue, offerIdValue, input) {
            ...
            const normalized = normalizeOfferInput(input, current.networkAccountId, current.code, normalizeOfferStatus(input.status));
            validateOfferReferences(snapshot, normalized);
        ...
        async createNetwork(identity, requestId, companyIdValue, input) {
            ...
            if (provider?.status !== 'active') {
                throw new ApiHttpError('CATALOG_PROVIDER_INVALID', 409, 'The selected network provider is unavailable.');
            }
            ...
            if (created === undefined) {
                throw new ApiHttpError('CATALOG_NETWORK_CONFLICT', 409, 'This network account already exists or the provider is unavailable.');
            }
            return created;
        },
        async updateNetwork(identity, requestId, companyIdValue, accountIdValue, input) {
            ...
            const normalized = normalizeNetworkInput(input, current.providerId, normalizeNetworkStatus(input.status));
        ...
        async updatePublisher(identity, requestId, companyIdValue, membershipIdValue, input) {
            ...
            if (managerMembershipId === undefined) {
                throw new ApiHttpError('CATALOG_MANAGER_INVALID', 403, 'An active Manager membership is required.');
            }
            const context = createRepositoryContext(identity, requestId, companyId);
            await requireActiveCompany(repository, context, companyId);
            const snapshot = scopeCatalogSnapshotForManager(await repository.getSnapshot(context, companyId), managerMembershipId, identity.actor.userId);
            const publisher = snapshot.publishers.find((item) => item.membershipId === membershipId);
            if (publisher === undefined) {
                throw new ApiHttpError('CATALOG_PUBLISHER_NOT_FOUND', 404, 'The Publisher membership was not found in the Manager scope.');
            }
            const payoutType = normalizePayoutType(input.payoutType);
            const payout = normalizePublisherPayout(payoutType, input.fixedPayoutAmountMinor, input.payoutCurrency);
            const assignedOfferIds = normalizeAssignedOfferIds(input.assignedOfferIds);
            const availableOfferIds = new Set(snapshot.offers.filter((offer) => offer.status !== 'archived').map((offer) => offer.id));
            if (assignedOfferIds.some((offerId) => !availableOfferIds.has(offerId))) {
                throw new ApiHttpError('CATALOG_OFFER_NOT_FOUND', 409, 'Publishers can only receive non-archived Offers assigned to the current Manager.');
            }
            if (publisher.membershipStatus !== 'active' && assignedOfferIds.length > 0) {
                throw new ApiHttpError('CATALOG_PUBLISHER_INVALID', 409, 'Only an active Publisher can receive Offer assignments.');
            }
            if (payoutType === 'per_offer') {
                const missingDefaultPayout = snapshot.offers.some((offer) => assignedOfferIds.includes(offer.id) &&
                    (offer.defaultPayoutAmountMinor === null || offer.payoutCurrency === null));
                if (missingDefaultPayout) {
                    throw new ApiHttpError('OFFER_ASSIGNMENT_PAYOUT_REQUIRED', 409, 'Every selected Offer requires a default payout before it can be assigned using per_offer mode.');
                }
            }
            ...
            const updated = await repository.updatePublisher(context, companyId, membershipId, normalized);
            if (updated === undefined) {
                throw new ApiHttpError('CATALOG_PUBLISHER_NOT_FOUND', 404, 'The Publisher membership was not found in the Manager scope.');
            }
            return updated;
...
//# sourceMappingURL=catalog-operations.service.js.map