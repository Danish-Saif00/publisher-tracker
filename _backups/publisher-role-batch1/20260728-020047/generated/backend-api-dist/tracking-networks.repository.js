...
function parseTrackingDomainStatus(value) {
    switch (value) {
...
function parseNetworkProviderStatus(value) {
    switch (value) {
        case 'active':
        case 'archived':
            return value;
...
function mapTrackingDomainRow(row) {
    return Object.freeze({
        id: row.id,
...
function mapNetworkProviderRow(row) {
    return Object.freeze({
        id: row.id,
        code: row.code,
        name: row.name,
...
`;
export function createTrackingNetworksRepository(database) {
    return Object.freeze({
        ...
        async listPlatformTrackingDomains(context, query) {
            return database.transaction(async (transaction) => {
                ...
                if (query.status !== undefined) {
                    conditions.push(`status = ${appendQueryValue(values, query.status)}::public.tracking_domain_status`);
                }
        ...
        async getTrackingDomain(context, domainId, companyId) {
            return database.transaction(async (transaction) => {
                ...
                const companyCondition = companyId === undefined
                    ? ''
        ...
        async listPlatformNetworkAccounts(context, query) {
            return database.transaction(async (transaction) => {
                ...
                const conditions = [];
                if (query.companyId !== undefined) {
                    conditions.push(`account.company_id = ${appendQueryValue(values, query.companyId)}::uuid`);
                }
                if (query.providerId !== undefined) {
                    conditions.push(`account.provider_id = ${appendQueryValue(values, query.providerId)}::uuid`);
                }
                if (query.status !== undefined) {
                    conditions.push(`account.status = ${appendQueryValue(values, query.status)}::public.network_account_status`);
                }
                const whereClause = conditions.length === 0 ? '' : `where ${conditions.join('\n                and ')}`;
                const result = await transaction.query({
...
//# sourceMappingURL=tracking-networks.repository.js.map