...
function parseMonetaryTotals(value) {
    return Object.freeze(readRequiredArray(value, 'monetaryTotals').map((entry) => {
        if (!isRecord(entry)) {
...
function parsePerformanceRows(value, fieldName) {
    return Object.freeze(readRequiredArray(value, fieldName).map((entry) => {
        if (!isRecord(entry)) {
        ...
        return Object.freeze({
            ...
            approvedConversions: readRequiredNumber(entry['approvedConversions'], 'approvedConversions'),
            monetaryTotals: parseMonetaryTotals(entry['monetaryTotals']),
        });
    }));
}
function parseReportingDashboard(value) {
    if (!isRecord(value)) {
    ...
    return Object.freeze({
        ...
        totals: Object.freeze({
            ...
            approvedConversions: readRequiredNumber(totals['approvedConversions'], 'totals.approvedConversions'),
...
function mapCustomizationRow(row) {
    return Object.freeze({
        ...
        linkIdentifierMode: row.link_identifier_mode,
        plainTextSharingEnabled: row.plain_text_sharing_enabled,
        restrictedSharePlatforms: row.restricted_share_platforms,
...
const customizationProjection = `
  ...
  updated_at
`;
export function createCompanyOperationsRepository(database) {
    return Object.freeze({
        ...
        async listOperationalEvents(context, companyId, input, scope) {
            return database.transaction(async (transaction) => {
                ...
                if (scope.managerMembershipId !== undefined && scope.managerUserId !== undefined) {
                    const managerMembershipPlaceholder = appendQueryValue(values, scope.managerMembershipId);
            ...
            `);
                ...
                if (input.from !== undefined) {
                    conditions.push(`event.created_at >= ${appendQueryValue(values, input.from)}::timestamptz`);
                }
                if (input.to !== undefined) {
                    conditions.push(`event.created_at < ${appendQueryValue(values, input.to)}::timestamptz`);
                }
        ...
        async completeSmtpTestEvent(context, companyId, smtpConfigurationId, eventId, status, errorCode) {
            return database.transaction(async (transaction) => {
                const result = await transaction.query({
...
//# sourceMappingURL=reporting-customization.repository.js.map