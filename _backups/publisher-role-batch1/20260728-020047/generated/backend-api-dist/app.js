import { assertCompanyAccess, AuthenticationError, AuthorizationError, } from '@affiliate-tracker/auth';
...
import { createApiCorsMiddleware, createApiRateLimitMiddleware, createApiSecurityHeadersMiddleware, } from './http-hardening.middleware.js';
...
const healthCheckHandler = (request, response) => {
...
function createReadinessHandler(readinessCheck, logger) {
    return async (request, response) => {
        ...
        try {
            ...
            response
                .status(503)
                .json(createErrorResponse('SERVICE_NOT_READY', 'The API is not ready to serve requests.', requestId));
        }
    };
}
const authenticatedIdentityHandler = (request, response) => {
...
const notFoundHandler = (request, response) => {
    response
        .status(404)
        .json(createErrorResponse('RESOURCE_NOT_FOUND', 'The requested resource was not found.', resolveRequestId(request)));
};
    ...
    return (error, request, response, next) => {
        ...
        if (error instanceof DatabaseError) {
            ...
            response
                ...
                .json(createErrorResponse(error.retriable ? 'DATABASE_UNAVAILABLE' : 'DATABASE_OPERATION_FAILED', error.retriable
                ? 'The database is temporarily unavailable.'
                : 'A database operation failed.', requestId));
            return;
        }
        ...
        response
            .status(500)
            .json(createErrorResponse('INTERNAL_SERVER_ERROR', 'An unexpected error occurred.', requestId));
    };
}
export function createApp(options) {
    ...
    if (options.config.swagger.enabled) {
        app.get(options.config.swagger.openApiJsonPath, (_request, response) => {
            response.status(200).json(openApiDocument);
        });
        app.use(options.config.swagger.documentationPath, swaggerUi.serve, swaggerUi.setup(openApiDocument, {
    ...
    app.get(joinApiPath(options.config.server.basePath, '/auth/me'), authenticationMiddleware, authenticatedIdentityHandler);
    authenticatedApiRouter.use(createCompanyManagementRouter({
        service: options.companyManagementService,
    }));
    authenticatedApiRouter.use(createCompanyInvitationsRouter({
        service: options.companyInvitationsService,
    }));
    authenticatedApiRouter.use(createTenantAdministrationRouter({
        service: options.tenantAdministrationService,
    }));
    authenticatedApiRouter.use(createBillingFoundationRouter({
        service: options.billingFoundationService,
    }));
    authenticatedApiRouter.use(createCatalogOperationsRouter({
        service: options.catalogOperationsService,
    }));
    authenticatedApiRouter.use(createTrackingNetworksRouter({
        service: options.trackingNetworksService,
    }));
    authenticatedApiRouter.use(createDuplicateFraudRouter({
        service: options.duplicateFraudService,
    }));
    authenticatedApiRouter.use(createConversionPostbacksRouter({
        service: options.conversionPostbacksService,
    }));
    authenticatedApiRouter.use(createCompanyOperationsRouter({
        service: options.companyOperationsService,
    }));
    authenticatedApiRouter.use(createFinalOperationsRouter({
        service: options.finalOperationsService,
    }));
    authenticatedApiRouter.use(createOffersPayoutRouter({
        service: options.offersPayoutService,
    }));
    authenticatedApiRouter.use(createTrackingLinksRouter({
        service: options.trackingLinksService,
    }));
    app.use(options.config.server.basePath, authenticationMiddleware, createPlatformSuperAdminScopeMiddleware(), authenticatedApiRouter);
