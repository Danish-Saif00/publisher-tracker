...
function parseMembershipStatus(
  value: string,
): CatalogPublisherRecord['membershipStatus'] {
  switch (value) {
    case 'invited':
    case 'active':
    case 'suspended':
...
const PUBLISHER_SELECT = `
  select
    ...
    coalesce(configuration.timezone, 'UTC') as timezone,
    coalesce(configuration.payout_type, 'per_offer') as payout_type,
    configuration.postback_url,
    ...
    greatest(membership.updated_at, coalesce(configuration.updated_at, membership.updated_at)) as updated_at
...
async function replaceOfferAssignments(
  transaction: DatabaseTransaction,
  context: CatalogRepositoryContext,
  offerId: string,
  ...
  await transaction.query({
    name: 'catalog-operations-revoke-removed-offer-assignments',
    text: `
  ...
  await transaction.query({
    name: 'catalog-operations-upsert-offer-assignments',
    text: `
          ...
          return Object.freeze({
            ...
            summary: Object.freeze({
              ...
              publishers: publishers.filter(
                (publisher) => publisher.membershipStatus !== 'revoked',
              ).length,
            }),
            providers,
            domains,
    ...
    async createOffer(context, companyId, input) {
      return database.transaction(
        async (transaction) => {
          ...
          await replaceOfferAssignments(
            transaction,
            context,
            offerId,
            input.publisherMembershipIds,
          );
          ...
          await writeAuditEvent(transaction, context, {
            ...
            metadata: {
              ...
              publisherCount: input.publisherMembershipIds.length,
            },
          });
      ...
      return database.transaction(
        async (transaction) => {
          const membershipResult = await transaction.query<
            { id: string } & Record<string, unknown>
          >({
            name: 'catalog-operations-verify-publisher',
            text: `
              select id
