...
function readNullableString(body: Record<string, unknown>, name: string): string | null | undefined {
  const value = body[name];
...
function readNullableNumber(body: Record<string, unknown>, name: string): number | null | undefined {
  const value = body[name];
...
function readOfferStatus(body: Record<string, unknown>, optional = false): CatalogOfferStatus | undefined {
...
function readOfferConfiguration(body: Record<string, unknown>) {
  return {
    ...
    publisherMembershipIds: readStringArray(body, 'publisherMembershipIds'),
  };
}
