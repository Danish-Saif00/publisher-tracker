...
function normalizeSearch(value: string | undefined): string | undefined {
  ...
  if (normalized.length > 160) {
    throw new ApiHttpError(
      'INVALID_QUERY_PARAMETER',
      400,
      'search cannot exceed 160 characters.',
    );
  }
  ...
  switch (value) {
    ...
    default:
      throw new ApiHttpError(
        'INVALID_QUERY_PARAMETER',
        400,
        'conversionStatus is unsupported.',
      );
  }
}
  ...
  switch (value) {
    ...
    default:
      throw new ApiHttpError(
        'INVALID_PATH_PARAMETER',
        400,
        'dimension must be networks, offers, or publishers.',
      );
  }
}
...
function normalizeCommonFilters(input: CommonFilterInput): NormalizedCommonFilters {
  ...
  const networkAccountId = normalizeOptionalUuid(
    input.networkAccountId,
    'networkAccountId',
  );
  const ownerMembershipId = normalizeOptionalUuid(
