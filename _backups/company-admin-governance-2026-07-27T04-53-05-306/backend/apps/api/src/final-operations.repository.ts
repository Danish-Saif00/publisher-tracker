...
function mapReportRow(row: ReportRow): PerformanceReportRow {
  return Object.freeze({
    ...
    approvedConversions: normalizeSafeInteger(
      row.approved_conversions,
      'approved_conversions',
    ),
    rejectedConversions: normalizeSafeInteger(
      row.rejected_conversions,
      'rejected_conversions',
    ),
    uncheckedConversions: normalizeSafeInteger(
      row.unchecked_conversions,
...
export function createFinalOperationsRepository(
  ...
  return Object.freeze<FinalOperationsRepository>({
    async listPerformanceReport(context, companyId, dimension, input, scope) {
      ...
      return database.transaction(
        async (transaction) => {
          ...
          if (scope.ownerUserId !== undefined) {
            conversionConditions.push(
              `conversion.owner_user_id = ${appendValue(
                values,
                scope.ownerUserId,
              )}::uuid`,
            );
          ...
          if (input.countryCode !== undefined) {
            conversionConditions.push(
              `${countryCodeExpression} = ${appendValue(
                values,
          ...
          if (input.device !== undefined) {
            conversionConditions.push(
              `${deviceExpression} = ${appendValue(values, input.device)}`,
            );
          }
          ...
          const searchPlaceholder =
            input.search === undefined
              ? null
              : appendValue(values, `%${input.search}%`);
          const statusPlaceholder =
            input.status === undefined
              ? null
              : appendValue(values, input.status);
    ...
    async createManualConversion(context, companyId, input) {
      return database.transaction(
        async (transaction) => {
          const clickResult = await transaction.query<ManualClickRow>({
            ...
            text: `
              ...
              limit 1
            `,
            values: [companyId, input.publicClickId],
          });
