...
export interface CreateConversionPostbacksRouterOptions {
  readonly service: ConversionPostbacksService;
}
...
function readOptionalEndpointStatus(
  value: unknown,
  errorCode: 'INVALID_QUERY_PARAMETER' | 'INVALID_REQUEST_BODY',
): NetworkPostbackEndpointStatus | undefined {
  switch (value) {
    case undefined:
      return undefined;
    case 'active':
...
function readOptionalCreateEndpointStatus(
  body: Record<string, unknown>,
): Extract<NetworkPostbackEndpointStatus, 'active' | 'paused'> | undefined {
  ...
  switch (value) {
    ...
    default:
      throw new ApiHttpError(
        ...
        400,
        'A new postback endpoint status must be active or paused.',
      );
  }
}

function readOptionalConversionStatus(value: unknown): ConversionStatus | undefined {
  switch (value) {
    case undefined:
      return undefined;
    case 'pending':
...
function readOptionalLimit(request: Request): number | undefined {
...
export function createConversionPostbacksRouter(
  options: CreateConversionPostbacksRouterOptions,
): Router {
  const router = Router();

  const createEndpointHandler: RequestHandler = async (request, response) => {
    const body = readBody(request);
    const requestInformation = resolveRequestInformation(request);

    const createStatus = readOptionalCreateEndpointStatus(body);

    const result = await options.service.createEndpoint(
      requestInformation.identity,
      requestInformation.requestId,
      readRouteParameter(request, 'companyId'),
      readRouteParameter(request, 'networkAccountId'),
      {
  ...
  const listEndpointsHandler: RequestHandler = async (request, response) => {
    const requestInformation = resolveRequestInformation(request);
    const status = readOptionalEndpointStatus(request.query['status'], 'INVALID_QUERY_PARAMETER');

    const endpoints = await options.service.listEndpoints(
      requestInformation.identity,
      requestInformation.requestId,
      readRouteParameter(request, 'companyId'),
      readRouteParameter(request, 'networkAccountId'),
  ...
  const updateEndpointHandler: RequestHandler = async (request, response) => {
    const body = readBody(request);
    const requestInformation = resolveRequestInformation(request);
    const status = readOptionalEndpointStatus(body['status'], 'INVALID_REQUEST_BODY');

    const endpoint = await options.service.updateEndpoint(
      requestInformation.identity,
      requestInformation.requestId,
      readRouteParameter(request, 'companyId'),
      readRouteParameter(request, 'networkAccountId'),
      readRouteParameter(request, 'endpointId'),
      {
  ...
  const rotateEndpointKeyHandler: RequestHandler = async (request, response) => {
    const requestInformation = resolveRequestInformation(request);

    const result = await options.service.rotateEndpointKey(
      requestInformation.identity,
      requestInformation.requestId,
      readRouteParameter(request, 'companyId'),
      readRouteParameter(request, 'networkAccountId'),
  ...
  const listConversionsHandler: RequestHandler = async (request, response) => {
    const requestInformation = resolveRequestInformation(request);
  ...
  const getConversionHandler: RequestHandler = async (request, response) => {
    const requestInformation = resolveRequestInformation(request);

    const conversion = await options.service.getConversion(
      requestInformation.identity,
      requestInformation.requestId,
      readRouteParameter(request, 'companyId'),
      readRouteParameter(request, 'conversionId'),
    );
