...
export interface CreateApiCorsMiddlewareOptions {
  readonly allowedOrigins: readonly string[];
}

export interface CreateApiRateLimitMiddlewareOptions {
  readonly maxRequests: number;
  readonly skipPaths?: readonly string[];
  readonly windowMs: number;
}

export interface CreateApiSecurityHeadersMiddlewareOptions {
  readonly environment: 'development' | 'production' | 'test';
  readonly openApiJsonPath: string;
  readonly swaggerDocumentationPath: string;
}
...
function appendVaryHeader(response: Response, value: string): void {
  response.vary(value);
}

function applyCorsResponseHeaders(response: Response, origin: string): void {
  response.setHeader('access-control-allow-origin', origin);
  response.setHeader('access-control-allow-methods', 'GET,HEAD,POST,PUT,PATCH,DELETE,OPTIONS');
  response.setHeader(
    'access-control-allow-headers',
    'authorization,content-type,idempotency-key,x-company-id,x-request-id',
  );
  response.setHeader(
...
export function createApiCorsMiddleware(options: CreateApiCorsMiddlewareOptions): RequestHandler {
  ...
  return (request, response, next): void => {
    ...
    if (!allowedOrigins.has(origin)) {
      response
        .status(403)
        .json(
          createErrorResponse('CORS_ORIGIN_DENIED', 'The request origin is not allowed.', request),
        );
...
export function createApiRateLimitMiddleware(
  options: CreateApiRateLimitMiddlewareOptions,
): RequestHandler {
    ...
    if (bucket.count > options.maxRequests) {
      ...
      response
        ...
        .json(
          createErrorResponse(
            'RATE_LIMIT_EXCEEDED',
            'Too many requests were received. Retry later.',
            request,
          ),
        );
...
function createContentSecurityPolicy(
  requestPath: string,
  options: CreateApiSecurityHeadersMiddlewareOptions,
): string {
  const isSwaggerRoute =
    requestPath === options.openApiJsonPath ||
...
export function createApiSecurityHeadersMiddleware(
  ...
  return (request, response, next): void => {
    response.setHeader(
      ...
      createContentSecurityPolicy(request.path, options),
    );
