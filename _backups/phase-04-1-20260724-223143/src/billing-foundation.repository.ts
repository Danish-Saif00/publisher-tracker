...
type BillingEntitlementRow = Readonly<{
  id: string;
  plan_id: string;
...
type BillingCompanyRow = Readonly<{
  id: string;
  status: string;
}> &
  Record<string, unknown>;

type CompanySubscriptionRow = Readonly<{
  id: string;
  company_id: string;
  plan_id: string;
  status: string;
...
export interface BillingFoundationRepository {
  createPlan(
    context: BillingRepositoryContext,
    input: BillingPlanWriteInput,
  ): Promise<BillingPlanRecord | undefined>;
  ...
  updatePlan(
    ...
    input: BillingPlanUpdateWriteInput,
  ): Promise<BillingPlanRecord | undefined>;

  countOpenSubscriptionsForPlan(context: BillingRepositoryContext, planId: string): Promise<number>;

  getCompany(
    context: BillingRepositoryContext,
    companyId: string,
  ): Promise<BillingCompanyRecord | undefined>;

  getCompanySubscription(
    context: BillingRepositoryContext,
    companyId: string,
  ): Promise<CompanySubscriptionRecord | undefined>;

  createCompanySubscription(
    context: BillingRepositoryContext,
    companyId: string,
    input: CompanySubscriptionWriteInput,
  ): Promise<CompanySubscriptionRecord | undefined>;
  ...
  updateCompanySubscription(
    ...
    companyId: string,
    expectedUpdatedAt: string,
    previousStatus: CompanySubscriptionStatus,
...
function parsePlanStatus(value: string): BillingPlanStatus {
  switch (value) {
    case 'active':
    case 'archived':
      return value;
...
function parseSubscriptionStatus(value: string): CompanySubscriptionStatus {
  switch (value) {
    case 'trialing':
    case 'active':
    case 'grace_period':
...
function mapEntitlementRow(row: BillingEntitlementRow): BillingPlanEntitlementRecord {
...
function mapPlanRow(
  row: BillingPlanRow,
  entitlements: readonly BillingPlanEntitlementRecord[],
): BillingPlanRecord {
  return Object.freeze({
    id: row.id,
    code: row.code,
...
function mapCompanyRow(row: BillingCompanyRow): BillingCompanyRecord {
  return Object.freeze({
    id: row.id,
...
function mapSubscriptionRow(row: CompanySubscriptionRow): CompanySubscriptionRecord {
  return Object.freeze({
    id: row.id,
...
function createDatabaseSessionContext(context: BillingRepositoryContext): DatabaseExecutionContext {
  return {
...
async function insertEntitlements(
  transaction: DatabaseTransaction,
  planId: string,
  entitlements: readonly NormalizedBillingEntitlementInput[],
): Promise<void> {
...
): Promise<readonly BillingPlanEntitlementRecord[]> {
  if (planIds.length === 0) {
    return Object.freeze([]);
  }

  const result = await transaction.query<BillingEntitlementRow>({
...
function attachEntitlements(
  ...
  entitlements: readonly BillingPlanEntitlementRecord[],
): readonly BillingPlanRecord[] {
  const entitlementsByPlan = new Map<string, BillingPlanEntitlementRecord[]>();
...
async function getPlanWithinTransaction(
  transaction: DatabaseTransaction,
  planId: string,
): Promise<BillingPlanRecord | undefined> {
  const result = await transaction.query<BillingPlanRow>({
...
export function createBillingFoundationRepository(
  database: DatabaseRuntime,
): BillingFoundationRepository {
  return Object.freeze<BillingFoundationRepository>({
    async createPlan(context, input): Promise<BillingPlanRecord | undefined> {
    ...
    async listPlans(context, status): Promise<readonly BillingPlanRecord[]> {
      return database.transaction(
        async (transaction) => {
          const result = await transaction.query<BillingPlanRow>({
    ...
    async getPlan(context, planId): Promise<BillingPlanRecord | undefined> {
      return database.transaction(
        async (transaction) => getPlanWithinTransaction(transaction, planId),
        {
          readOnly: true,
          sessionContext: createDatabaseSessionContext(context),
        },
    ...
    async countOpenSubscriptionsForPlan(context, planId): Promise<number> {
      return database.transaction(
        async (transaction) => {
    ...
    async getCompany(context, companyId): Promise<BillingCompanyRecord | undefined> {
      return database.transaction(
        async (transaction) => {
          const result = await transaction.query<BillingCompanyRow>({
    ...
    async getCompanySubscription(
      ...
      return database.transaction(
        async (transaction) => {
          const result = await transaction.query<CompanySubscriptionRow>({
