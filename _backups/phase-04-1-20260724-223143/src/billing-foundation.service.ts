...
import type { BillingFoundationRepository } from './billing-foundation.repository.js';
import type {
  BillingAccessRecord,
...
const ALLOWED_SUBSCRIPTION_TRANSITIONS: Readonly<
  Record<CompanySubscriptionStatus, readonly CompanySubscriptionStatus[]>
> = {
...
export interface BillingFoundationService {
  createPlan(
  ...
  ): Promise<readonly BillingPlanRecord[]>;
...
function normalizePlanCode(value: string): string {
  ...
  if (
    ...
    throw new ApiHttpError(
      ...
      400,
      'Billing plan code must contain 2 to 80 lowercase letters, numbers, or single underscores.',
    );
  }
...
function normalizeOptionalNullableText(
  ...
  if (normalizedValue.length === 0 || normalizedValue.length > maximumLength) {
    throw new ApiHttpError(
      'INVALID_REQUEST_BODY',
      400,
      `${fieldName} must contain 1 to ${String(maximumLength)} characters when provided.`,
    );
  }
...
function normalizeInteger(
  ...
  if (!Number.isInteger(value) || value < minimumValue || value > maximumValue) {
    throw new ApiHttpError(
      'INVALID_REQUEST_BODY',
      400,
      `${fieldName} must be an integer between ${String(minimumValue)} and ${String(maximumValue)}.`,
    );
  }
...
function normalizeBillingInterval(value: BillingInterval): BillingInterval {
  switch (value) {
    case 'monthly':
    case 'annual':
      return value;
...
function normalizePlanStatus(
  ...
  errorCode: 'INVALID_REQUEST_BODY' | 'INVALID_QUERY_PARAMETER',
): BillingPlanStatus {
  switch (value) {
    case 'active':
    case 'archived':
...
function normalizeSubscriptionStatus(value: CompanySubscriptionStatus): CompanySubscriptionStatus {
  switch (value) {
    case 'trialing':
    case 'active':
    case 'grace_period':
...
function normalizeEntitlements(
  input: readonly BillingEntitlementInput[],
): readonly NormalizedBillingEntitlementInput[] {
  if (input.length > MAX_ENTITLEMENTS) {
    throw new ApiHttpError(
      'INVALID_REQUEST_BODY',
      400,
      `A billing plan cannot contain more than ${String(MAX_ENTITLEMENTS)} entitlements.`,
    );
  }
  ...
  const entitlements = input.map((entitlement) => {
    ...
    if (key.length < 2 || key.length > 80 || !ENTITLEMENT_KEY_PATTERN.test(key)) {
      throw new ApiHttpError(
        'INVALID_REQUEST_BODY',
        400,
        'Entitlement keys must contain 2 to 80 lowercase letters, numbers, or underscores and start with a letter.',
      );
    }
    ...
    if (seenKeys.has(key)) {
      throw new ApiHttpError(
        ...
        409,
        `The entitlement key "${key}" is duplicated.`,
      );
    }
    ...
    if (!enabled && limitValue !== null) {
      throw new ApiHttpError(
        'INVALID_REQUEST_BODY',
        400,
        `Disabled entitlement "${key}" cannot define a limitValue.`,
      );
    }
...
function assertDateAfter(laterValue: string, earlierValue: string, fieldName: string): void {
  if (new Date(laterValue).getTime() <= new Date(earlierValue).getTime()) {
    throw new ApiHttpError(
      'BILLING_SUBSCRIPTION_DATE_INVALID',
      400,
      `${fieldName} must be later than ${earlierValue}.`,
    );
  }
}
...
async function requirePlan(
  repository: BillingFoundationRepository,
  context: BillingRepositoryContext,
  planId: string,
): Promise<BillingPlanRecord> {
  ...
  if (plan === undefined) {
    throw new ApiHttpError(
      ...
      404,
      'The requested billing plan was not found.',
    );
  }
...
async function requireActivePlan(
  ...
  if (plan.status !== 'active') {
    throw new ApiHttpError(
      ...
      409,
      'An archived billing plan cannot be assigned or modified.',
    );
  }
...
function assertSubscriptionTransition(
  currentStatus: CompanySubscriptionStatus,
  requestedStatus: CompanySubscriptionStatus,
): void {
  ...
  if (!ALLOWED_SUBSCRIPTION_TRANSITIONS[currentStatus].includes(requestedStatus)) {
    throw new ApiHttpError(
      ...
      409,
      `A ${currentStatus} subscription cannot transition to ${requestedStatus}.`,
    );
  }
}

function normalizeExternalReference(value: string | null | undefined): string | null | undefined {
...
function createAccessRecord(
  ...
  subscription: CompanySubscriptionRecord | undefined,
  now: string,
): BillingAccessRecord {
...
async function buildCompanyBillingSnapshot(
  repository: BillingFoundationRepository,
  context: BillingRepositoryContext,
  companyId: string,
  now: string,
  ...
  const plan =
    subscription === undefined ? undefined : await repository.getPlan(context, subscription.planId);
...
function createPlanUpdate(input: UpdateBillingPlanInput): BillingPlanUpdateWriteInput {
  const changedFields: string[] = [];

  const name =
    input.name === undefined ? undefined : normalizeRequiredText(input.name, 'name', 2, 160);
  ...
  const entitlements =
    input.entitlements === undefined ? undefined : normalizeEntitlements(input.entitlements);
...
function plansAreEquivalent(plan: BillingPlanRecord, input: BillingPlanUpdateWriteInput): boolean {
  const scalarValuesAreEqual =
...
function subscriptionsAreEquivalent(
  current: CompanySubscriptionRecord,
  next: CompanySubscriptionWriteInput,
): boolean {
  return (
...
export function createBillingFoundationService(
  repository: BillingFoundationRepository,
  options: BillingFoundationServiceOptions = {},
): BillingFoundationService {
  const getNow = options.now ?? (() => new Date());

  return Object.freeze<BillingFoundationService>({
    async createPlan(identity, requestId, input): Promise<BillingPlanRecord> {
      assertPlatformSuperAdmin(identity.subject);

      const normalizedInput: BillingPlanWriteInput = Object.freeze({
        ...
        priceAmountMinor: normalizeInteger(
      ...
      if (plan === undefined) {
        throw new ApiHttpError(
          ...
          409,
          'A billing plan with this code already exists.',
        );
      }
    ...
    async listPlans(identity, requestId, input): Promise<readonly BillingPlanRecord[]> {
      assertPlatformSuperAdmin(identity.subject);
    ...
    async getPlan(identity, requestId, planIdValue): Promise<BillingPlanRecord> {
      assertPlatformSuperAdmin(identity.subject);
    ...
    async updatePlan(identity, requestId, planIdValue, input): Promise<BillingPlanRecord> {
      assertPlatformSuperAdmin(identity.subject);
      ...
      if (plan.status === 'archived') {
        throw new ApiHttpError(
          'BILLING_PLAN_ARCHIVED',
          409,
          'An archived billing plan is immutable.',
        );
      }
      ...
      if (plansAreEquivalent(plan, update)) {
        throw new ApiHttpError(
          ...
          409,
          'The billing plan already contains the requested values.',
        );
      }
      ...
      if (update.status === 'archived' && update.entitlements !== undefined) {
        throw new ApiHttpError(
          'INVALID_REQUEST_BODY',
          400,
          'Billing entitlements cannot be replaced while archiving a plan.',
        );
      }

      if (update.status === 'archived') {
        const openSubscriptionCount = await repository.countOpenSubscriptionsForPlan(
          context,
          planId,
        );
        ...
        if (openSubscriptionCount > 0) {
          throw new ApiHttpError(
            'BILLING_PLAN_IN_USE',
            409,
            'A billing plan with an open company subscription cannot be archived.',
          );
        }
      }
      ...
      if (updatedPlan === undefined) {
        throw new ApiHttpError(
          ...
          409,
          'The billing plan changed before this request could be completed.',
        );
      }
    ...
    async createCompanySubscription(
      ...
      if (company.status === 'archived') {
        throw new ApiHttpError(
          ...
          409,
          'An archived company cannot receive a billing subscription.',
        );
      }
      ...
      if (existingSubscription !== undefined) {
        throw new ApiHttpError(
          ...
          409,
          'The company already has a billing subscription.',
        );
      }
      ...
      const startsAt =
        input.startsAt === undefined ? now : normalizeDateTime(input.startsAt, 'startsAt');
      ...
      const subscription = await repository.createCompanySubscription(
        context,
        companyId,
        Object.freeze({
          planId,
      ...
      if (subscription === undefined) {
        throw new ApiHttpError(
          'BILLING_SUBSCRIPTION_CONFLICT',
          409,
          'The company subscription changed before this request could be completed.',
        );
      }
    ...
    async getPlatformCompanyBilling(
      identity,
      requestId,
      companyIdValue,
    ): Promise<CompanyBillingSnapshot> {
    ...
    async updateCompanySubscription(
      ...
      if (company.status === 'archived') {
        throw new ApiHttpError(
          'BILLING_COMPANY_ARCHIVED',
          409,
          'An archived company subscription cannot be modified.',
        );
      }
      ...
      if (current === undefined) {
        throw new ApiHttpError(
          ...
          404,
          'The company billing subscription was not found.',
        );
      }
      ...
      if (!hasInput) {
        throw new ApiHttpError(
          'INVALID_REQUEST_BODY',
          400,
          'At least one company subscription field must be provided.',
        );
      }
      ...
      const targetStatus =
        input.status === undefined ? current.status : normalizeSubscriptionStatus(input.status);
      ...
      const currentPeriodEndsAt: string | null =
        requestedCurrentPeriodEndsAt === undefined
      ...
      if (targetStatus === 'grace_period') {
        ...
        if (suppliedGraceEndsAt !== undefined && suppliedGraceEndsAt !== null) {
          ...
          if (targetPlan.gracePeriodDays === 0) {
            throw new ApiHttpError(
              'BILLING_SUBSCRIPTION_DATE_INVALID',
              400,
              'graceEndsAt is required because the billing plan has no default grace period.',
            );
          }
        ...
        throw new ApiHttpError(
          'BILLING_SUBSCRIPTION_DATE_INVALID',
          400,
          'graceEndsAt can only be set for a grace_period subscription.',
        );
      }

      const next: CompanySubscriptionWriteInput = Object.freeze({
        planId: targetPlanId,
        status: targetStatus,
      ...
      if (subscriptionsAreEquivalent(current, next)) {
        throw new ApiHttpError(
          ...
          409,
          'The company subscription already contains the requested values.',
        );
      }

      const updatedSubscription = await repository.updateCompanySubscription(
        context,
        companyId,
        current.updatedAt,
        current.status,
