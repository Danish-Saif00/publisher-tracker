...
function normalizeMembershipIds(values: readonly string[]): readonly string[] {
  const normalized = values.map((value) => normalizeUuid(value, 'Publisher membership ID'));

  if (new Set(normalized).size !== normalized.length) {
    throw new ApiHttpError(
      'INVALID_REQUEST_BODY',
      400,
      'publisherMembershipIds cannot contain duplicate values.',
    );
  }
...
function normalizeOfferInput(
  ...
  return Object.freeze({
    ...
    publisherMembershipIds: normalizeMembershipIds(input.publisherMembershipIds),
  });
}
...
function validateOfferReferences(
  ...
  const eligiblePublisherIds = new Set(
    snapshot.publishers
      .filter(
        (publisher) =>
          publisher.membershipStatus === 'active' && publisher.userStatus === 'active',
      )
      .map((publisher) => publisher.membershipId),
  );

  if (input.publisherMembershipIds.some((membershipId) => !eligiblePublisherIds.has(membershipId))) {
    throw new ApiHttpError(
      'CATALOG_PUBLISHER_INVALID',
      409,
      'Offers can only be assigned to active Publisher memberships.',
    );
  }
}
...
export function createCatalogOperationsService(
  ...
  return Object.freeze<CatalogOperationsService>({
    async getSnapshot(identity, requestId, companyIdValue) {
      ...
      return repository.getSnapshot(context, companyId);
    },
