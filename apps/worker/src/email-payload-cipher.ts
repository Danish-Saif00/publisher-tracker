...
function parsePayload(
  ...
  try {
    ...
    throw new Error(
      'Decrypted email notification payload is not valid JSON.',
      {
        cause: error,
      },
    );
  ...
  if (!isRecord(parsed)) {
    throw new Error(
      'Decrypted email notification payload must be a JSON object.',
    );
  }
  ...
  return Object.freeze<EmailPayloadCipher>({
    ...
    decrypt(
      ...
      if (
        ...
        throw new Error(
          'Stored email payload encryption metadata is invalid.',
        );
      }
      const decipher =
        createDecipheriv(
