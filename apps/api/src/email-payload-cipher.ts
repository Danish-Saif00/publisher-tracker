...
export type EmailNotificationPayload = Readonly<Record<string, unknown>>;

export interface EmailPayloadCipher {
  encrypt(payload: EmailNotificationPayload): EncryptedEmailPayload;
  decrypt(payload: EncryptedEmailPayload): EmailNotificationPayload;
}
...
function serializePayload(payload: EmailNotificationPayload): string {
  ...
  if (
    ...
    throw new Error(
      'Email notification payload must serialize to between 2 and 48000 characters.',
    );
  }
...
function parsePayload(serialized: string): EmailNotificationPayload {
  let parsed: unknown;
...
export function createEmailPayloadCipher(
  ...
  return Object.freeze<EmailPayloadCipher>({
    encrypt(payload: EmailNotificationPayload): EncryptedEmailPayload {
