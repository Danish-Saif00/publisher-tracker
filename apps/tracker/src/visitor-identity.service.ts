...
} from './visitor-identity.types.js';
...
export interface VisitorIdentityServiceOptions {
  readonly cookieName: string;
  readonly maxAgeDays: number;
  readonly signingSecret: string;
  readonly secureCookies: boolean;
  readonly now?: () => Date;
  readonly createVisitorId?: () => string;
}

interface ParsedVisitorCookie {
...
function normalizeMaxAgeDays(value: number): number {
...
function createSignature(signingSecret: string, payload: string): string {
  return createHmac('sha256', signingSecret).update(payload).digest('base64url');
}

function signaturesMatch(expectedValue: string, actualValue: string): boolean {
...
function createIdentityResult(
  visitorId: string,
  source: VisitorIdentitySource,
...
export function createVisitorIdentityService(
  options: VisitorIdentityServiceOptions,
): VisitorIdentityService {
  ...
  const renewalWindowSeconds = Math.min(
    30 * SECONDS_PER_DAY,
    Math.max(SECONDS_PER_DAY, Math.floor(maxAgeSeconds / 4)),
  );
  const now = options.now ?? (() => new Date());
  const createVisitorId = options.createVisitorId ?? randomUUID;

  return Object.freeze<VisitorIdentityService>({
    resolveVisitorIdentity(cookieHeader): ResolvedVisitorIdentity {
      const nowDate = now();
      ...
      const parsedCookie = parseVisitorCookie(
        readCookieValue(cookieHeader, cookieName),
        signingSecret,
      );

      if (parsedCookie !== undefined) {
        const cookieIsCurrent =
          parsedCookie.expiresAtSeconds > nowSeconds &&
        ...
        if (cookieIsCurrent) {
          if (parsedCookie.expiresAtSeconds - nowSeconds > renewalWindowSeconds) {
            return createIdentityResult(
              parsedCookie.visitorId,
              'existing_cookie',
              null,
              parsedCookie.expiresAtSeconds,
            );
          }
